...
use ::ml::{quantized::QTensor, DType, Device, Module, Tensor, WithDType};
...
fn actual_index(t: &Tensor, dim: usize, index: i64) -> ::ml::Result<usize> {
    let dim = t.dim(dim)?;
    if 0 <= index {
        let index = index as usize;
        if dim <= index {
            ::ml::bail!("index {index} is too large for tensor dimension {dim}")
        }
        Ok(index)
    } else {
        if (dim as i64) < -index {
            ::ml::bail!("index {index} is too low for tensor dimension {dim}")
        }
        Ok((dim as i64 + index) as usize)
    }
}

fn actual_dim(t: &Tensor, dim: i64) -> ::ml::Result<usize> {
    let rank = t.rank();
    if 0 <= dim {
        let dim = dim as usize;
        if rank <= dim {
            ::ml::bail!("dimension index {dim} is too large for tensor rank {rank}")
        }
        Ok(dim)
    } else {
        if (rank as i64) < -dim {
            ::ml::bail!("dimension index {dim} is too low for tensor rank {rank}")
        }
...
impl PyQTensor {
    ...
    fn matmul_t(&self, lhs: &PyTensor) -> PyResult<PyTensor> {
        let qmatmul = ::ml::quantized::QMatMul::from_arc(self.0.clone()).map_err(wrap_err)?;
...
fn load_safetensors(path: &str, py: Python<'_>) -> PyResult<Py<PyAny>> {
    let res = ::ml::safetensors::load(path, &Device::Cpu).map_err(wrap_err)?;
    let res = res
        .into_iter()
...
fn save_safetensors(
    ...
    ::ml::safetensors::save(&tensors, path).map_err(wrap_err)
}
...
fn load_ggml<'py>(
    ...
    let device = device.unwrap_or(PyDevice::Cpu).as_device()?;
    let ggml =
        ::ml::quantized::ggml_file::Content::read(&mut file, &device).map_err(wrap_err)?;
    let tensors = ggml
        .tensors
        .into_iter()
...
fn softmax(tensor: PyTensor, dim: i64) -> PyResult<PyTensor> {
    ...
    let sm = ml_nn::ops::softmax(&tensor.0, dim).map_err(wrap_err)?;
    Ok(PyTensor(sm))
}
...
fn silu(tensor: PyTensor) -> PyResult<PyTensor> {
    let s = ml_nn::ops::silu(&tensor.0).map_err(wrap_err)?;
    Ok(PyTensor(s))
}
...
#[cfg(feature = "onnx")]
fn ml_onnx_m(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
