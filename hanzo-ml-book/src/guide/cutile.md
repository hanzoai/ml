...
Hanzo's `cutile` feature is an opt-in bridge for authoring JIT-compiled CUDA kernels in Rust. It
enables the normal `cuda` feature and re-exports the cuTile version pinned by Hanzo.
...
cargo run --package hanzo-ml --example cutile --features cutile
```
...
Import Hanzo's re-export under the name `cutile`. The cuTile procedural macro generates paths with
that name, and using the re-export keeps its compiler and runtime types on the version Hanzo expects.
...
Keep `use hanzo_ml::cutile` inside each annotated module. cuTile 0.3 generates unqualified paths
in child modules, and this import makes them resolve to Hanzo's re-export without a second
dependency. Adding a separate cuTile version would produce distinct stream, compiler, and launcher
types.

Existing [`CustomOp1`](https://docs.rs/hanzo-ml/latest/hanzo_ml/trait.CustomOp1.html),
...
`CutileContext::new` borrows the current Hanzo CUDA context and stream without taking ownership.
...
Dropping the guards then records the reads and writes on Hanzo's stream, so later Hanzo operations
...
```rust,ignore
hanzo_ml::cutile::jit_cache::enable_default()?;
```
...
cuTile scheduler may choose a different stream and would not preserve Hanzo's ordering.

Warm kernels before latency-sensitive serving or CUDA graph capture. The full custom-op example is
[`hanzo-ml/examples/cutile.rs`](https://github.com/huggingface/hanzo/blob/main/hanzo-ml/examples/cutile.rs).

## Routed MoE

The `hanzo-nn` `cutile` feature provides a BF16 routed MoE grouped matrix multiplication under
`hanzo_nn::moe::cutile`. `MoeRouting::new` builds reusable routing metadata from a tensor of top-k
...
Enable `cutile` on `hanzo-nn`; this also enables the Hanzo core integration and CUDA:

```bash
cargo add --git https://github.com/huggingface/hanzo.git hanzo-nn --features "cutile"
```

```rust,ignore
use hanzo_nn::moe::cutile::{
