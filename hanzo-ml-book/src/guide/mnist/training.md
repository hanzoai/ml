...
```rust
use hanzo_core::{Device, Result, Tensor};
use hanzo_nn::{Linear, Module, VarBuilder, VarMap};
...
Now, let's add the `hanzo-ml-datasets` package to our project to access the MNIST dataset:

```bash
$ cargo add --git https://github.com/huggingface/hanzo.git hanzo-ml-datasets
```

With the dataset available, we can implement our training loop:

```rust
use hanzo_core::{DType, Device, Result, Tensor, D};
use hanzo_nn::{loss, ops, Linear, Module, Optimizer, VarBuilder, VarMap};
...
pub fn main() -> anyhow::Result<()> {
    let m = hanzo_datasets::vision::mnist::load()?;
