...
use ml::{safetensors::Load, DType, Device, IndexOp, Tensor, D};
use ml_nn::{ops::softmax, VarBuilder};
pub use ml_transformers::models::whisper::{self as m, Config};
...
impl Model {
    ...
    pub fn encoder_forward(&mut self, x: &Tensor, flush: bool) -> ml::Result<Tensor> {
        match self {
    ...
    pub fn decoder_final_linear(&self, x: &Tensor) -> ml::Result<Tensor> {
        match self {
...
impl Decoder {
    ...
    fn decode(&mut self, mel: &Tensor, t: f64) -> anyhow::Result<DecodingResult> {
        ...
        for i in 0..sample_len {
            ...
            let prob = softmax(&logits, ml::D::Minus1)?
    ...
    pub fn load(md: ModelData) -> anyhow::Result<Self> {
        ...
        let model = if md.quantized {
            let vb = ml_transformers::quantized_var_builder::VarBuilder::from_gguf_buffer(
                &md.weights,
                &device,
            )?;
...
pub fn detect_language(model: &mut Model, tokenizer: &Tokenizer, mel: &Tensor) -> Result<u32, E> {
    ...
    let probs = ml_nn::ops::softmax(&logits, D::Minus1)?;
...
pub fn token_id(tokenizer: &Tokenizer, token: &str) -> ml::Result<u32> {
    match tokenizer.token_to_id(token) {
        None => ml::bail!("no token-id for {token}"),
        Some(id) => Ok(id),
    }
}
