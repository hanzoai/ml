...
fn run_print(
    ...
    if full {
        hanzo_ml_core::display::set_print_options_full();
    }
    if let Some(line_width) = line_width {
        hanzo_ml_core::display::set_line_width(line_width)
    }
    ...
    match format {
        Format::Npz => {
            let tensors = hanzo_ml_core::npy::NpzTensors::new(file)?;
        ...
        Format::Safetensors => {
            ...
            let tensors = unsafe { hanzo_ml_core::safetensors::MmapedSafetensors::new(file)? };
        ...
        Format::Pth => {
            let pth_file = hanzo_ml_core::pickle::PthTensors::new(file, None)?;
            let names = if names.is_empty() {
                pth_file
                    .tensor_infos()
                    .keys()
        ...
        Format::Pickle => {
            hanzo_ml_core::bail!("pickle format is not supported for print")
        }
        Format::Ggml => {
            let mut file = std::fs::File::open(file)?;
            let content = hanzo_ml_core::quantized::ggml_file::Content::read(&mut file, device)?;
            let names = if names.is_empty() {
    ...
    match format {
        ...
        Format::Safetensors => {
            ...
            for (name, view) in tensors.iter() {
                ...
                let dtype = match hanzo_ml_core::DType::try_from(dtype) {
        ...
        Format::Pth => {
            let mut tensors = hanzo_ml_core::pickle::read_pth_tensor_info(file, verbose, None)?;
        ...
        Format::Pickle => {
            ...
            let mut stack = hanzo_ml_core::pickle::Stack::empty();
...
fn run_quantize_safetensors(
    ...
    for in_file in in_files.iter() {
        let in_tensors = hanzo_ml_core::safetensors::load(in_file, &Device::Cpu)?;
...
fn run_dequantize(
    ...
    hanzo_ml_core::safetensors::save(&tensors, out_file)?;
    Ok(())
}
...
fn run_quantize(
    ...
    if in_files.is_empty() {
        hanzo_ml_core::bail!("no specified input files")
    }
    if let Some(extension) = out_file.extension() {
        if extension == "safetensors" {
            hanzo_ml_core::bail!("the generated file cannot use the safetensors extension")
        }
    }
    ...
    if in_files.len() != 1 {
        hanzo_ml_core::bail!("only a single in-file can be used when quantizing gguf files")
    }
