...
fn bf16_cpu_op_sweep() -> Result<()> {
    ...
    let checks: Vec<(&str, f32)> = vec![
        (
            "silu",
            rel(&hanzo_nn::ops::silu(&x32)?, &hanzo_nn::ops::silu(&xb)?)?,
        ),
        (
            "softmax",
            rel(
                &hanzo_nn::ops::softmax_last_dim(&x32)?,
                &hanzo_nn::ops::softmax_last_dim(&xb)?,
            )?,
        ),
    ...
    let rms32 = hanzo_nn::ops::rms_norm(&x32, &w32, 1e-6)?;
    let rmsb = hanzo_nn::ops::rms_norm(&xb, &w32.to_dtype(DType::BF16)?, 1e-6)?;
