...
use hanzo_ml::cuda_backend::cudarc::driver::{CudaSlice, DevicePtr, DevicePtrMut, DeviceRepr};
use hanzo_ml::cuda_backend::{CudaDType, CudaDevice};
use hanzo_ml::cutile;
use hanzo_ml::{CudaStorage, DType, Layout, Result, Storage, Tensor};
...
impl MoeRouting {
    ...
    pub fn new(topk_ids: &Tensor, num_experts: usize) -> Result<Self> {
        ...
        if num_tokens == 0 || top_k == 0 {
            hanzo_ml::bail!("cuTile MoE routing requires at least one route")
        }
        if num_experts == 0 || num_experts > MAX_EXPERTS {
            hanzo_ml::bail!("cuTile MoE supports between 1 and {MAX_EXPERTS} experts")
        }
        if !matches!(topk_ids.dtype(), DType::U32 | DType::I32) {
            hanzo_ml::bail!("cuTile MoE top-k ids must be u32 or i32")
        }
        let device = topk_ids.device().as_cuda_device()?.clone();
        let num_routes = num_tokens
            .checked_mul(top_k)
            .ok_or_else(|| hanzo_ml::Error::msg("cuTile MoE route count overflow"))?;
        ...
        let padded_bound = num_routes
            .checked_add(
                num_experts
                    ...
                    .ok_or_else(|| hanzo_ml::Error::msg("cuTile MoE route padding overflow"))?,
            )
            .ok_or_else(|| hanzo_ml::Error::msg("cuTile MoE route padding overflow"))?;
        ...
        if !layout.is_contiguous() {
            hanzo_ml::bail!("cuTile MoE top-k ids must be contiguous")
        }
...
fn routed_grouped_matmul_inner(
    ...
    if input.dtype() != DType::BF16 || expert_weights.dtype() != DType::BF16 {
        hanzo_ml::bail!("cuTile routed grouped matmul requires bf16 input and weights")
    }
    let (input_rows, input_k) = input.dims2()?;
    let (num_experts, n_size, weight_k) = expert_weights.dims3()?;
    if n_size == 0 || weight_k == 0 {
        hanzo_ml::bail!("cuTile routed grouped matmul requires nonzero N and K dimensions")
    }
    ...
    let input_len = input_rows
        .checked_mul(input_k)
        .ok_or_else(|| hanzo_ml::Error::msg("cuTile routed grouped matmul input size overflow"))?;
    let weight_len = num_experts
        .checked_mul(n_size)
        .and_then(|size| size.checked_mul(weight_k))
        .ok_or_else(|| hanzo_ml::Error::msg("cuTile routed grouped matmul weight size overflow"))?;
    ...
    if let Some(route_weights) = route_weights {
        if route_weights.dtype() != DType::F32 {
            hanzo_ml::bail!("cuTile MoE route weights must be f32")
        }
    ...
    let output_len = routing
        ...
        .ok_or_else(|| hanzo_ml::Error::msg("cuTile routed grouped matmul output size overflow"))?;
    ...
    let grid_x = routing
        ...
        .ok_or_else(|| hanzo_ml::Error::msg("cuTile routed grouped matmul launch grid overflow"))?;
    let generics = vec![
        config.bm.to_string(),
        config.bn.to_string(),
        config.bk.to_string(),
...
fn launch_alignment<T: CudaDType + DeviceRepr>(
    ...
    let storage = match storage {
        ...
        _ => hanzo_ml::bail!("cuTile MoE top-k ids must be on CUDA"),
    };
    ...
    let topk_offset = layout
        ...
        .ok_or_else(|| hanzo_ml::Error::msg("cuTile MoE top-k id offset overflow"))?;
    let topk_ptr = topk_ptr
        .checked_add(topk_offset as u64)
        .ok_or_else(|| hanzo_ml::Error::msg("cuTile MoE top-k id pointer overflow"))?;
    ...
    if status != 0 {
        hanzo_ml::bail!("cuTile MoE route alignment CUDA launch failed with error {status}")
    }
    drop((
        cumsum_guard,
        count_guard,
...
fn require_contiguous(layout: &Layout, name: &str) -> Result<()> {
    if !layout.is_contiguous() {
        hanzo_ml::bail!("cuTile routed grouped matmul {name} must be contiguous")
    }
    Ok(())
}
...
fn cuda_storage<'a>(storage: &'a Storage, name: &str) -> Result<&'a CudaStorage> {
    match storage {
        ...
        _ => hanzo_ml::bail!("cuTile routed grouped matmul {name} must be on CUDA"),
    }
}
...
fn same_device(tensor: &Tensor, device: &CudaDevice, name: &str) -> Result<()> {
    ...
    if tensor_device.id() != device.id() {
        hanzo_ml::bail!("cuTile routed grouped matmul {name} is on a different CUDA device")
    }
    Ok(())
}
...
fn checked_i32(value: impl TryInto<i32>, name: &str) -> Result<i32> {
    value
        .try_into()
        .map_err(|_| hanzo_ml::Error::msg(format!("cuTile MoE {name} exceeds i32 range")))
}

fn checked_u32(value: impl TryInto<u32>, name: &str) -> Result<u32> {
    value
        .try_into()
        .map_err(|_| hanzo_ml::Error::msg(format!("cuTile MoE {name} exceeds u32 range")))
}
