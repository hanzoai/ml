...
impl hanzo_ml::CustomOp3 for Sdpa {
    ...
    fn metal_fwd(
        ...
        // The vector kernel reduces over a single query row, so it is only correct at q_seq == 1;
        // everything longer goes through the full kernel. The split is exclusive by construction.
        ...
        if supports_sdpa_vector {
            ...
            if self.softcapping != 1. {
                hanzo_ml::bail!("SDPA full requires softcapping to be disabled (1.0)");
            }
            ...
            let (mask_type, mask_buffer, mask_strides) = if let Some(mask) = &self.mask {
                ...
                let mask_buffer = match &**mask_s {
                    hanzo_ml::Storage::Metal(m) => m.buffer(),
                    _ => hanzo_ml::bail!("Expected metal device for mask"),
                };
                ...
                if mask_type != itype {
                    hanzo_ml::bail!("Mask type {mask_type:?} must match q type {itype:?}");
                }
