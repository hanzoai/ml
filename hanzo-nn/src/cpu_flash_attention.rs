...
use hanzo::{Device, Result, Storage, Tensor, WithDType};
    ...
    T: WithDType + Sum + num_traits::real::Real,
{
    ...
    let q_data: &[T] = if let Storage::Cpu(cpu) = &*q_guard {
        ...
        return Err(hanzo::Error::Msg("Expected CPU storage for q".into()));
    };
    ...
    let k_data: &[T] = if let Storage::Cpu(cpu) = &*k_guard {
        ...
        return Err(hanzo::Error::Msg("Expected CPU storage for k".into()));
    };
    ...
    let v_data: &[T] = if let Storage::Cpu(cpu) = &*v_guard {
        ...
        return Err(hanzo::Error::Msg("Expected CPU storage for v".into()));
    };
    ...
    let mask_data: Option<&[T]> = if let Some(mask_guard) = &mask_guard {
        ...
        if let Storage::Cpu(cpu) = &**mask_guard {
            ...
            return Err(hanzo::Error::Msg("Expected CPU storage for mask".into()));
        }
    } else {
        None
    };
