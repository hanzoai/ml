...
#[cfg(feature = "cuda")]
use hanzo::cuda_backend::kernels::ffi;
#[allow(unused_imports)]
use hanzo::quantized::{self, QTensor};
...
pub fn moe_gemm(
    ...
    >(
        ...
        let data_type = match input.dtype() {
            ...
            DType::BF16 => 1,
            _ => {
                hanzo::bail!("moe_gemm_wmma only accepts f16/bf16 inputs")
            }
        };

        let (input, _) = input.storage_and_layout();
        let input = match &*input {
            hanzo::Storage::Cuda(c) => c.as_cuda_slice::<T>()?,
            _ => hanzo::bail!("input must be a cuda tensor"),
        };

        let (weights, _) = weights.storage_and_layout();
        let weights = match &*weights {
            hanzo::Storage::Cuda(c) => c.as_cuda_slice::<T>()?,
            _ => hanzo::bail!("weight must be a cuda tensor"),
        };

        let (sorted_token_ids, _) = sorted_token_ids.storage_and_layout();
        let sorted_token_ids = match &*sorted_token_ids {
            hanzo::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?,
            _ => hanzo::bail!("sorted_token_ids must be a cuda tensor"),
        };

        let (experts_ids, _) = experts_ids.storage_and_layout();
        let experts_ids = match &*experts_ids {
            hanzo::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?,
            _ => hanzo::bail!("experts_ids must be a cuda tensor"),
        };
        ...
        let topk_weights_ptr = if let Some(topk_weights) = &topk_weights {
            ...
            let topk_weights = match &*topk_weights {
                ...
                _ => hanzo::bail!("topk_weights must be a cuda tensor"),
            };
        ...
        use hanzo::op::BackpropOp;
        let output = hanzo::CudaStorage::wrap_cuda_slice(output, dev.clone());
        let output = Tensor::from_storage(
            hanzo::Storage::Cuda(output),
            (size_m, size_n),
            BackpropOp::none(),
            false,
        );
    ...
    match input.dtype() {
        ...
        _ => {
            hanzo::bail!("moe_gemm only accepts f16/bf16 inputs")
        }
    }
}
...
pub fn moe_gemm(
    ...
    hanzo::bail!("moe_gemm is only implemented for the cuda backend")
}
...
pub fn moe_gemm_gguf(
    ...
    match input.dtype() {
        ...
        _ => {
            hanzo::bail!("moe_gemm_gguf only accepts f32 inputs")
        }
    }
}
...
pub fn moe_gemm_gguf(
    ...
    hanzo::bail!("moe_gemm_gguf is only implemented for the cuda backend")
}
