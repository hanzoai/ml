...
use ml::Module;
use ml::{quantized::QTensor, DType, Result, Tensor, D};
use ml_nn::{linear_no_bias, moe, Activation, Linear, VarBuilder};
use std::sync::Arc;
...
impl FusedMoe {
    ...
    pub fn forward(&self, xs: &Tensor, is_prefill: bool) -> Result<Tensor> {
        ...
        let routing_weights =
            ml_nn::ops::softmax_last_dim(&router_logits.to_dtype(DType::F32)?)?;
        ...
        let gate = gate_up
            .narrow(ml::D::Minus1, 0, self.w_size_n)?
            .contiguous()?;
        let up = gate_up
            .narrow(ml::D::Minus1, self.w_size_n, self.w_size_n)?
            .contiguous()?;
