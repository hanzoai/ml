...
use ml::{DType, IndexOp, Layout, Module, Result, Shape, Tensor, D};
use ml_nn::{conv1d, Conv1d, ConvTranspose1d, VarBuilder};
...
fn pad1d(xs: &Tensor, pad_l: usize, pad_r: usize, mode: PadMode) -> Result<Tensor> {
    match mode {
        ...
        PadMode::Reflect => ml::bail!("pad-mode 'reflect' is not supported"),
...
pub fn conv_transpose1d_weight_norm(
    ...
    kernel_size: usize,
    bias: bool,
    config: ml_nn::ConvTranspose1dConfig,
    vb: VarBuilder,
...
impl ml::CustomOp2 for CodebookEncode {
    ...
    fn cpu_fwd(
        &self,
        lhs_storage: &ml::CpuStorage,
        lhs_layout: &Layout,
        rhs_storage: &ml::CpuStorage,
        rhs_layout: &Layout,
    ) -> Result<(ml::CpuStorage, Shape)> {
        use rayon::prelude::*;

        let (lhs_dim1, lhs_dim2) = lhs_layout.shape().dims2()?;
        let (rhs_dim1, rhs_dim2) = rhs_layout.shape().dims2()?;
        if lhs_dim2 != rhs_dim2 {
            ml::bail!("CodebookEncode, mismatch on last dim, {lhs_layout:?} {rhs_layout:?}");
        }
        if lhs_dim2 == 0 {
            ml::bail!("CodebookEncode, empty last dim {lhs_layout:?}")
        }
        let lhs = match lhs_layout.contiguous_offsets() {
            None => ml::bail!("CodebookEncode, lhs has to be contiguous, got {lhs_layout:?}"),
            Some((o1, o2)) => {
        ...
        let rhs = match rhs_layout.contiguous_offsets() {
            None => ml::bail!("CodebookEncode, rhs has to be contiguous, got {rhs_layout:?}"),
            Some((o1, o2)) => {
...
impl EuclideanCodebook {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            inited,
            cluster_size,
            embed: ml_nn::Embedding::new(embed, cfg.codebook_dim()),
            embed_avg,
            c2,
        })
    }
...
impl EncodecLSTM {
    pub fn new(dim: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        for layer_idx in 0..cfg.num_lstm_layers {
            let config = ml_nn::LSTMConfig {
                layer_idx,
                ..Default::default()
            };
            let lstm = ml_nn::lstm(dim, dim, config, vb.clone())?;
            layers.push(lstm)
        }
        Ok(Self { layers })
    }
...
impl EncodecConvTranspose1d {
    fn new(
        ...
        let cfg = ml_nn::ConvTranspose1dConfig {
            stride,
            ..Default::default()
        };
    ...
    pub fn new(
        ...
        let norm = match cfg.norm_type {
            ...
            NormType::TimeGroupNorm => {
                let gn = ml_nn::group_norm(1, out_c, 1e-5, vb.pp("norm"))?;
                Some(gn)
            }
        };
        Ok(Self {
