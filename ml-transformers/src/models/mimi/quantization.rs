...
use ml::{IndexOp, Layout, Result, Shape, Tensor, D};
use ml_nn::{linear, Linear, VarBuilder};
...
impl EuclideanCodebook {
    ...
    pub fn decode(&self, indexes: &Tensor) -> Result<Tensor> {
        ...
        // let ys = ml_nn::Embedding::new(self.embedding.clone(), self.dim).forward(xs)?;
...
impl ResidualVectorQuantization {
    ...
    pub fn decode(&self, xs: &Tensor) -> Result<Tensor> {
        if self.layers.is_empty() {
            ml::bail!("empty layers in ResidualVectorQuantization")
        }
...
pub struct ResidualVectorQuantizer {
    ...
    input_proj: Option<ml_nn::Conv1d>,
    output_proj: Option<ml_nn::Conv1d>,
}
