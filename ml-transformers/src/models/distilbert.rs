...
use ml::{DType, Device, Result, Tensor};
use ml_nn::{Embedding, Module, VarBuilder};
...
impl Module for HiddenActLayer {
    fn forward(&self, xs: &Tensor) -> ml::Result<Tensor> {
...
impl Embeddings {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let word_embeddings =
            ml_nn::embedding(config.vocab_size, config.dim, vb.pp("word_embeddings"))?;
        let position_embeddings = ml_nn::embedding(
...
impl MultiHeadSelfAttention {
    fn forward(&self, hidden_states: &Tensor, attention_mask: &Tensor) -> Result<Tensor> {
        ...
        let weights = ml_nn::ops::softmax(&scores, ml::D::Minus1)?;
...
impl DistilBertLMPredictionHead {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        let init_ws = ml_nn::init::DEFAULT_KAIMING_NORMAL;
        ...
        let init_bs = ml_nn::Init::Uniform {
            lo: -bound,
            up: bound,
        };
