...
use ml::{DType, Device, Module, Result, Tensor};
use ml_nn::kv_cache::KvCache;
use ml_nn::Activation;
use std::io::Write;
use std::sync::Arc;
    ...
    pub fn from_gguf(ct: &gguf_file::Content) -> Result<Self> {
        ...
        let get_u32 = |key: &str| -> Result<usize> {
            metadata
                ...
                .map(|v| v as usize)
                .ok_or_else(|| {
                    ml::Error::Msg(format!("Missing or invalid metadata key: {}", key))
                })
        };
...
impl RmsNorm {
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let hidden_size = x.dim(ml::D::Minus1)?;
        let x = x.to_dtype(internal_dtype)?;
        let norm_x = (x.sqr()?.sum_keepdim(ml::D::Minus1)? / hidden_size as f64)?;
...
impl QuantizedAttention {
    fn new(
        ...
        use ml::quantized::{GgmlDType, QTensor};
    ...
    fn forward(&mut self, x: &Tensor, mask: Option<&Tensor>, offset: usize) -> Result<Tensor> {
        ...
        attn_weights = ml_nn::ops::softmax_last_dim(&attn_weights)?;
...
impl QuantizedModelForCausalLM {
    pub fn from_gguf<P: AsRef<std::path::Path>>(path: P, device: &Device) -> Result<Self> {
        ...
        let embed_tokens = ml_nn::Embedding::new(embed_tensor_gpu, config.hidden_size);
