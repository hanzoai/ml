...
use ml_nn::{kv_cache::KvCache, Activation, VarBuilder};
use std::sync::Arc;
...
impl SmolLM3Attention {
    pub(crate) fn new(
        ...
        if use_sliding_window {
            ml::bail!("sliding window is not supported")
        }
...
#[derive(Debug, Clone)]
pub struct Model {
    pub(crate) embed_tokens: ml_nn::Embedding,
...
impl Model {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let embed_tokens =
            ml_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb.pp("model.embed_tokens"))?;
