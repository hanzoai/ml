...
impl BertSelfAttention {
    ...
    fn forward(&self, hidden_states: &Tensor, attention_mask: &Tensor) -> Result<Tensor> {
        ...
        let attention_probs = {
            ...
            ml_nn::ops::softmax(&attention_scores, ml::D::Minus1)?
        };
        let attention_probs = self.dropout.forward(&attention_probs)?;

        let context_layer = attention_probs.matmul(&value_layer)?;
        let context_layer = context_layer.transpose(1, 2)?.contiguous()?;
        let context_layer = context_layer.flatten_from(ml::D::Minus2)?;
        Ok(context_layer)
    }
}
...
fn get_extended_attention_mask(attention_mask: &Tensor, dtype: DType) -> Result<Tensor> {
    let attention_mask = match attention_mask.rank() {
        ...
        _ => ml::bail!("Wrong shape for input_ids or attention_mask"),
    };
