...
use ml::{DType, Device, Error, IndexOp, Module, Result, Tensor, D};
use ml_nn::{layer_norm, Activation, LayerNorm, VarBuilder};
use std::sync::Arc;
...
impl Config {
    ...
    pub fn new_1_5_b_v5(embed_dim: EmbedDim) -> Self {
        ...
        Self {
            ...
            activation_fn: ml_nn::Activation::Silu,
            vocab_size: 151646,
            hidden_size: 1536,
...
impl RotaryEmbedding {
    ...
    fn apply_rotary_emb_qkv(&self, q: &Tensor, k: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        let q_embed = ml_nn::rotary_emb::rope(&q.contiguous()?, &cos, &sin)?;
        let k_embed = ml_nn::rotary_emb::rope(&k.contiguous()?, &cos, &sin)?;
        Ok((q_embed, k_embed))
    }
}
...
pub struct Embeddings {
    ...
    embeddings: ml_nn::Embedding,
    // following are specifically for 400M
    token_type_embeddings: Option<ml_nn::Embedding>,
...
impl Embeddings {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let (embeddings, token_type_embeddings, layer_norm, position_ids) = match cfg.variant {
            ModelVariant::Large => (
                ml_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb.pp("embed_tokens"))?,
                None,
                None,
                None,
            ),
            ModelVariant::Small => {
                ...
                let weight = vb.pp("LayerNorm").get_with_hints(
                    cfg.hidden_size,
                    "weight",
                    ml_nn::Init::Const(1.0),
                )?;
                let bias = vb.pp("LayerNorm").get_with_hints(
                    cfg.hidden_size,
                    "bias",
                    ml_nn::Init::Const(0.0),
                )?;
                let dev = bias.device().clone();

                let layer_norm = ml_nn::LayerNorm::new(weight, bias, cfg.norm_eps);
...
impl Model {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        // let embed_tokens =
        //     ml_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb_m.pp("embed_tokens"))?;
