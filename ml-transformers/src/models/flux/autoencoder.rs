use ml::{Result, Tensor, D};
use ml_nn::{conv2d, group_norm, Conv2d, GroupNorm, VarBuilder};
...
fn scaled_dot_product_attention(q: &Tensor, k: &Tensor, v: &Tensor) -> Result<Tensor> {
    ...
    ml_nn::ops::softmax_last_dim(&attn_weights)?.matmul(v)
}
...
impl ml::Module for ResnetBlock {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let h = xs
            .apply(&self.norm1)?
            .apply(&ml_nn::Activation::Swish)?
            .apply(&self.conv1)?
            .apply(&self.norm2)?
            .apply(&ml_nn::Activation::Swish)?
            .apply(&self.conv2)?;
...
impl ml_nn::Module for DiagonalGaussian {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl ml::Module for AutoEncoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
