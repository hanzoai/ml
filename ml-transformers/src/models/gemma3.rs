...
use ml::{DType, Device, Module, Result, Tensor, D};
use ml_nn::{linear_b as linear, Activation, Linear, VarBuilder};
...
#[derive(Debug, Clone)]
enum KvCache {
    Normal(ml_nn::kv_cache::KvCache),
    Rotating(ml_nn::kv_cache::RotatingKvCache),
}
...
impl Attention {
    fn new(
        ...
        let kv_cache = if let Some(sliding_window) = sliding_window {
            KvCache::Rotating(ml_nn::kv_cache::RotatingKvCache::new(2, sliding_window))
        } else {
            KvCache::Normal(ml_nn::kv_cache::KvCache::new(
                2,
