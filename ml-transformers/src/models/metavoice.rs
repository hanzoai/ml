...
use ml::{DType, Device, Error as E, IndexOp, Module, Result, Tensor, D};
use ml_nn::{embedding, linear_b, rms_norm, Embedding, Linear, RmsNorm, VarBuilder};
...
pub mod speaker_encoder {
    ...
    impl Module for Model {
        fn forward(&self, xs: &Tensor) -> Result<Tensor> {
            ...
            // This is different from the Python transformers version as ml LSTM is batch first.
            let xs = xs.t()?;
            let mut xs = xs.clone();
    ...
    impl BPE {
        pub fn from_json(json: &serde_json::Value, end_of_text: usize) -> Result<Self> {
            let json = match json.as_object() {
                None => ml::bail!("json value is not an object"),
                Some(json) => json,
            };
            let re = match json.get("pat_str") {
                None => ml::bail!("json object has no pat_str field"),
                Some(pat_str) => match pat_str.as_str() {
                    None => ml::bail!("pat_str field is not a string"),
                    Some(pat_str) => fancy_regex::Regex::new(pat_str).map_err(E::wrap)?,
                },
            };
            let offset = match json.get("offset") {
                None => ml::bail!("json object has no offset field"),
                Some(offset) => match offset.as_u64() {
                    None => ml::bail!("offset field is not a positive int"),
            ...
            let mergeable_ranks = match json.get("mergeable_ranks") {
                None => ml::bail!("json object has no mergeable_ranks field"),
                Some(mr) => match mr.as_object() {
                    None => ml::bail!("mergeable_ranks is not an object"),
                    Some(mr) => mr,
                },
            };
            for (key, value) in mergeable_ranks.iter() {
                let value = match value.as_u64() {
                    None => ml::bail!("mergeable_ranks '{key}' is not a u64"),
...
pub mod gpt {
    ...
    enum Norm {
        RMSNorm(ml_nn::RmsNorm),
        LayerNorm(ml_nn::LayerNorm),
    }
    ...
    impl Norm {
        fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
            match cfg.norm_type {
                NormType::RMSNorm => {
                    let rms_norm = ml_nn::rms_norm(cfg.n_embd, cfg.rmsnorm_eps, vb)?;
                    Ok(Self::RMSNorm(rms_norm))
                }
                NormType::LayerNorm => {
                    let ln_cfg = ml_nn::LayerNormConfig {
                        affine: cfg.bias,
                        ..Default::default()
                    };
                    let layer_norm = ml_nn::layer_norm(cfg.n_embd, ln_cfg, vb)?;
    ...
    impl SelfAttention {
        fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
            ...
            if cfg.attn_kernel_type != AttnKernelType::TorchAttn {
                ml::bail!("only TorchAttn is supported")
            }
            if cfg.kv_cache_enabled {
                ml::bail!("kv_cache_enabled=true is not supported")
            }
            ...
            let slf = match cfg.nonlinearity_type {
                ...
                NonLinearityType::Swiglu => {
                    ...
                    let swiglu_multiple_of = match cfg.swiglu_multiple_of {
                        None => ml::bail!("swiglu-multiple-of has to be set"),
                        Some(smo) => smo,
                    };
    ...
    impl Model {
        pub fn new(cfg: Config, vb: VarBuilder) -> Result<Self> {
            ...
            for (idx, vocab_size) in cfg.vocab_sizes.iter().enumerate() {
                let wte = ml_nn::embedding(*vocab_size, cfg.n_embd, vb_w.pp(idx))?;
                wtes.push(wte)
            }
            let wpe = ml_nn::embedding(cfg.block_size, cfg.n_embd, vb_t.pp("wpe"))?;
...
pub mod transformer {
    ...
    impl Module for FeedForward {
        fn forward(&self, xs: &Tensor) -> Result<Tensor> {
            let _enter = self.span.enter();
            let swiglu = (ml_nn::ops::silu(&xs.apply(&self.w1)?)? * xs.apply(&self.w3))?;
    ...
    impl Attention {
        ...
        fn forward(&mut self, xs: &Tensor, _pos: usize, mask: &Tensor) -> Result<Tensor> {
            ...
            let attn_weights = ml_nn::ops::softmax_last_dim(&attn_weights)?;
