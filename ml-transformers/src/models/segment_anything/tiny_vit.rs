...
use ml::{IndexOp, Result, Tensor, D};
use ml_nn::{Conv2dConfig, Module, VarBuilder};
...
impl Conv2dBN {
    fn new(in_: usize, out: usize, ks: usize, cfg: Conv2dConfig, vb: VarBuilder) -> Result<Self> {
        let c = ml_nn::conv2d_no_bias(in_, out, ks, cfg, vb.pp("c"))?;
        let bn = ml_nn::batch_norm(out, 1e-5, vb.pp("bn"))?;
...
impl Mlp {
    fn new(in_: usize, hidden: usize, vb: VarBuilder) -> Result<Self> {
        let norm = ml_nn::layer_norm(in_, 1e-5, vb.pp("norm"))?;
...
impl Attention {
    fn new(
        ...
        let h = dh + nh_kd * 2;
        let norm = ml_nn::layer_norm(dim, 1e-5, vb.pp("norm"))?;
...
impl Module for Attention {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let attn = {
            ...
            ml_nn::ops::softmax_last_dim(&attn)?
        };
        let attn = {
            let _enter = self.span_matmul.enter();
            attn.matmul(&v)?
...
pub struct TinyViT {
    ...
    // norm_head: ml_nn::LayerNorm,
    // head: ml_nn::Linear,
    neck_conv1: ml_nn::Conv2d,
    neck_ln1: super::LayerNorm2d,
    neck_conv2: ml_nn::Conv2d,
...
impl TinyViT {
    pub fn new(
        ...
        // let norm_head = ml_nn::layer_norm(last_embed_dim, 1e-5, vb.pp("norm_head"))?;
        // let head = ml_nn::linear(last_embed_dim, num_classes, vb.pp("head"))?;
        let neck_conv1 =
            ml_nn::conv2d_no_bias(last_embed_dim, 256, 1, Default::default(), vb.pp("neck.0"))?;
        let neck_ln1 = super::LayerNorm2d::new(256, 1e-6, vb.pp("neck.1"))?;
        let cfg = ml_nn::Conv2dConfig {
            padding: 1,
            ..Default::default()
        };
        let neck_conv2 = ml_nn::conv2d_no_bias(256, 256, 3, cfg, vb.pp("neck.2"))?;
