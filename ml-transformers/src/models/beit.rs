...
use ml_nn::{layer_norm, LayerNorm, Linear, Module, VarBuilder};
...
fn linear(vb: VarBuilder, in_dim: usize, out_dim: usize, bias: bool) -> Result<Linear> {
    if bias {
        ml_nn::linear(in_dim, out_dim, vb)
    } else {
        ml_nn::linear_no_bias(in_dim, out_dim, vb)
    }
}
...
impl Module for Attention {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let attn = ml_nn::ops::softmax(&attn, D::Minus1)?;
...
impl PatchEmbed {
    fn new(vb: VarBuilder, patch_size: usize, in_chans: usize, embed_dim: usize) -> Result<Self> {
        let config = ml_nn::Conv2dConfig {
            stride: patch_size,
            ..Default::default()
        };
        let proj = ml_nn::conv2d(in_chans, embed_dim, patch_size, config, vb.pp("proj"))?;
        Ok(Self {
            proj,
...
impl Module for PatchEmbed {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        if (h % patch_h) != 0 {
            ml::bail!("image height {h} is not a multiple of patch height {patch_h}")
        }
        if (w % patch_w) != 0 {
            ml::bail!("image width {w} is not a multiple of patch width {patch_w}")
        }
