...
use ml::{Context, Module, ModuleT, Result, Tensor, D};
use ml_nn::{conv2d_no_bias, layer_norm, Activation, Conv2dConfig, VarBuilder};
...
impl SegformerOverlapPatchEmbeddings {
    fn new(
        ...
        let layer_norm =
            ml_nn::layer_norm(hidden_size, config.layer_norm_eps, vb.pp("layer_norm"))?;
        Ok(Self {
            projection,
            layer_norm,
        })
...
impl Module for SegformerEfficientSelfAttention {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let attention_scores = ml_nn::ops::softmax_last_dim(&attention_scores)?;
...
struct SegformerLayer {
    layer_norm_1: ml_nn::LayerNorm,
    attention: SegformerAttention,
    layer_norm_2: ml_nn::LayerNorm,
    mlp: SegformerMixFFN,
}
...
struct SegformerEncoder {
    ...
    layer_norms: Vec<ml_nn::LayerNorm>,
}
...
struct SegformerDecodeHead {
    ...
    linear_fuse: ml_nn::Conv2d,
    batch_norm: ml_nn::BatchNorm,
    classifier: ml_nn::Conv2d,
}
...
impl SegformerDecodeHead {
    fn new(config: &Config, num_labels: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let batch_norm = ml_nn::batch_norm(
            config.decoder_hidden_size,
            config.layer_norm_eps,
            vb.pp("batch_norm"),
        )?;
