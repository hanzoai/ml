...
impl CoreAttention {
    ...
    fn forward(
        ...
        let attention_probs = ml_nn::ops::softmax_last_dim(&attention_scores)?;
...
impl SelfAttention {
    ...
    fn forward(
        ...
        if !self.multi_query_attention {
            ml::bail!("only multi_query_attention=true is supported")
        }
...
#[derive(Debug, Clone)]
struct Block {
    input_layernorm: ml_nn::LayerNorm,
    self_attention: SelfAttention,
    post_attention_layernorm: ml_nn::LayerNorm,
    mlp: MLP,
...
struct Transformer {
    layers: Vec<Block>,
    final_layernorm: Option<ml_nn::LayerNorm>,
    rotary_emb: RotaryEmbedding,
}
...
#[derive(Debug, Clone)]
struct Embedding {
    word_embeddings: ml_nn::Embedding,
    fp32_residual_connection: bool,
}

impl Embedding {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let word_embeddings = ml_nn::embedding(
