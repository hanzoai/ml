...
use ml_nn::{embedding, rms_norm, Activation, Embedding, Linear, Module, RmsNorm, VarBuilder};
...
impl CustomOp1 for NonZero {
    ...
    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        ...
        let result = match storage {
            ml::CpuStorage::U8(vs) => self.nonzero(vs, layout),
            ml::CpuStorage::U32(vs) => self.nonzero(vs, layout),
            ml::CpuStorage::I16(vs) => self.nonzero(vs, layout),
            ml::CpuStorage::I32(vs) => self.nonzero(vs, layout),
            ml::CpuStorage::I64(vs) => self.nonzero(vs, layout),
            ml::CpuStorage::BF16(vs) => self.nonzero(vs, layout),
            ml::CpuStorage::F16(vs) => self.nonzero(vs, layout),
            ml::CpuStorage::F32(vs) => self.nonzero(vs, layout),
            ml::CpuStorage::F64(vs) => self.nonzero(vs, layout),
            ml::CpuStorage::F8E4M3(vs) => self.nonzero(vs, layout),
            // Dummy types don't support nonzero operation
            ml::CpuStorage::F6E2M3(_) => {
                return Err(
                    ml::Error::UnsupportedDTypeForOp(ml::DType::F6E2M3, "nonzero").bt(),
                )
            }
            ml::CpuStorage::F6E3M2(_) => {
                return Err(
                    ml::Error::UnsupportedDTypeForOp(ml::DType::F6E3M2, "nonzero").bt(),
                )
            }
            ml::CpuStorage::F4(_) => {
                return Err(ml::Error::UnsupportedDTypeForOp(ml::DType::F4, "nonzero").bt())
            }
            ml::CpuStorage::F8E8M0(_) => {
                return Err(
                    ml::Error::UnsupportedDTypeForOp(ml::DType::F8E8M0, "nonzero").bt(),
                )
            }
        };
...
impl NonZeroOp for Tensor {
    fn nonzero(&self) -> Result<Tensor> {
        if !self.is_contiguous() {
            return Err(ml::Error::RequiresContiguous { op: "nonzero" });
        }
        let original_device = self.device();
        self.to_device(&ml::Device::Cpu)?
...
impl DeepSeekV2RotaryEmbedding {
    ...
    pub fn new(cfg: &DeepSeekV2RopeConfig, dtype: DType, dev: &Device) -> Result<Self> {
        match &cfg.rope_scaling {
            Some(DeepSeekV2RopeScaling::LinearOrDynamic {
                scaling_type: _,
                factor: _,
            }) => ml::bail!("linear and dynamic rope are not implemented yet!"),
    ...
    pub fn forward(
        ...
        let q_embed = ml_nn::rotary_emb::rope_i(&q.contiguous()?, &cos, &sin)?;
        let k_embed = ml_nn::rotary_emb::rope_i(&k.contiguous()?, &cos, &sin)?;
...
impl Attention {
    fn new(
        ...
        let q = match cfg.q_lora_rank {
            ...
            None => QProj::Plain(ml_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.num_attention_heads * q_head_dim,
                vb.pp("q_proj"),
            )?),
        ...
        let kv_a_proj_with_mqa = ml_nn::linear_b(
            cfg.hidden_size,
    ...
    fn forward(
        ...
        let attn_out = {
            ...
            let att = ml_nn::ops::softmax_last_dim(&att)?;
...
impl Mlp {
    fn new(
        ...
        Ok(Self {
            gate: ml_nn::linear_no_bias(hidden_size, intermediate_size, vb.pp("gate_proj"))?,
            up: ml_nn::linear_no_bias(hidden_size, intermediate_size, vb.pp("up_proj"))?,
            down: ml_nn::linear_no_bias(intermediate_size, hidden_size, vb.pp("down_proj"))?,
            act: cfg.hidden_act,
        })
    }
...
impl MoeGate {
    ...
    fn forward(&self, xs: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        let scores = match self.cfg.scoring_func {
            ScoringFunc::Softmax => ml_nn::ops::softmax_last_dim(&logits)?,
        };
...
impl DeepSeekV2 {
    pub fn new(cfg: &DeepSeekV2Config, vb: VarBuilder) -> Result<Self> {
        ...
        let lm_head = if !cfg.tie_word_embeddings {
            ml_nn::linear_no_bias(cfg.hidden_size, cfg.vocab_size, vb.pp("lm_head"))?
        } else {
            ml_nn::Linear::new(embed_tokens.embeddings().clone(), None)
        };
