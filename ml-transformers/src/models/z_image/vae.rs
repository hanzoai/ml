...
use ml::{Module, Result, Tensor, D};
use ml_nn::{conv2d, group_norm, Conv2d, Conv2dConfig, GroupNorm, VarBuilder};
...
impl Attention {
    fn new(channels: usize, num_groups: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let to_q = ml_nn::linear(channels, channels, vb.pp("to_q"))?;
        let to_k = ml_nn::linear(channels, channels, vb.pp("to_k"))?;
        let to_v = ml_nn::linear(channels, channels, vb.pp("to_v"))?;
        let to_out = ml_nn::linear(channels, channels, vb.pp("to_out").pp("0"))?;
        Ok(Self {
            group_norm,
            to_q,
            to_k,
