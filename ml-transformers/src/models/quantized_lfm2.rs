...
use ml::quantized::QMatMul;
use ml::{bail, DType, Device, IndexOp, Result, Tensor};
use ml_nn::{Conv1d, Conv1dConfig, Embedding, Module};
...
impl Module for Mlp {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        self.w2.forward(&(ml_nn::ops::silu(&w1)? * w3)?)
    }
}
...
impl AttentionLayer {
    fn apply_rotary_emb(&self, x: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        ml_nn::rotary_emb::rope(&x.contiguous()?, &cos, &sin)
    }
