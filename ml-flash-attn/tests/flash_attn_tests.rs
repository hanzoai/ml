use anyhow::Result;
use ml::{DType, Device, IndexOp, Tensor, D};
...
fn flash_attn_acausal() -> Result<()> {
    ...
    let ys2 = {
        ...
        ml_flash_attn::flash_attn(&q, &k, &v, 0.5, false)?.transpose(1, 2)?
    };
...
fn flash_attn_acausal_softcap() -> Result<()> {
    ...
    let ys2 = {
        ...
        let k = k.transpose(1, 2)?;
        let v = v.transpose(1, 2)?;
        ml_flash_attn::flash_attn_alibi_windowed_softcap(
            &q,
            &k,
            &v,
            None,            //  alibi_slopes //
...
fn flash_attn_varlen() -> Result<()> {
    ...
    let ys = {
        ...
        ml_flash_attn::flash_attn_varlen(
