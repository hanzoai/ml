...
use ml::quantized::GgmlType;
use ml::{CpuStorage, Device, Layout, Module, Result, Shape, Tensor, D};
...
impl Benchmark for QMatMul {
    type PreProcessData = (ml::quantized::QMatMul, Tensor);
    type RunResult = Tensor;
    fn preprocess() -> Result<Self::PreProcessData> {
        let zeros = vec![ml::quantized::k_quants::BlockQ4_0::zeros(); 4096 * 11008 / 32];
        let mm = ml::quantized::QTensor::new(
            ml::quantized::QStorage::Cpu(Box::new(zeros)),
            (4096, 11008),
        )?;
        let mm = ml::quantized::QMatMul::from_qtensor(mm)?;
