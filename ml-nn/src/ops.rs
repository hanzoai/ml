...
use ml::{CpuStorage, DType, Layout, Module, Result, Shape, Tensor, D};
use rayon::prelude::*;
...
///
/// ```rust
/// use ml::{Tensor, Device, test_utils::to_vec2_round};
/// let a = Tensor::new(&[[0f32, 1., 0., 1.], [-2., 2., 3., -3.]], &Device::Cpu)?;
/// let a = ml_nn::ops::softmax(&a, 1)?;
/// assert_eq!(
///     to_vec2_round(&a, 4)?,
///     &[
...
/// # Ok::<(), ml::Error>(())
/// ```
pub fn softmax<D: ml::shape::Dim>(xs: &Tensor, dim: D) -> Result<Tensor> {
...
pub fn log_softmax<D: ml::shape::Dim>(xs: &Tensor, d: D) -> Result<Tensor> {
...
impl ml::CustomOp1 for Sigmoid {
    fn name(&self) -> &'static str {
        "sigmoid"
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        use ml::backend::BackendStorage;

        fn fwd<T: num_traits::Float>(v: T) -> T {
            (v.neg().exp() + T::one()).recip()
        }

        // FIXME: using `ml::map_dtype` causes compilation errors.
        let storage = match storage {
            CpuStorage::BF16(slice) => {
                CpuStorage::BF16(ml::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F16(slice) => {
                CpuStorage::F16(ml::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F32(slice) => {
                CpuStorage::F32(ml::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F64(slice) => {
                CpuStorage::F64(ml::cpu_backend::unary_map(slice, layout, fwd))
            }
            _ => Err(ml::Error::UnsupportedDTypeForOp(
                storage.dtype(),
                self.name(),
            ))?,
        };
    ...
    fn cuda_fwd(
        &self,
        storage: &ml::CudaStorage,
        layout: &Layout,
    ) -> Result<(ml::CudaStorage, Shape)> {
        use ml::backend::BackendStorage;
        use ml::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchConfig, PushKernelArg, ValidAsZeroBits,
        };
        use ml::cuda_backend::SlicePtrOrNull;
        use ml::cuda_backend::{kernel_name, kernels, Map1, WrapErr};
        use ml::{CudaDevice, WithDType};
        ...
        impl Map1 for S {
            fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
                ...
                ml::builder_arg!(builder, el_count, dims.len());
    ...
    fn metal_fwd(
        &self,
        storage: &ml::MetalStorage,
        layout: &Layout,
    ) -> Result<(ml::MetalStorage, Shape)> {
        use ml::backend::BackendStorage;
        use ml::MetalError;
        ...
        if layout.is_contiguous() {
            ...
            use ml_metal_kernels::unary::strided;
            let kernel_name = match dtype {
                DType::F16 => strided::sigmoid::HALF,
                DType::F32 => strided::sigmoid::FLOAT,
                DType::BF16 => strided::sigmoid::BFLOAT,
                dtype => {
                    ml::bail!("Metal strided unary sigmoid {dtype:?} not implemented")
                }
            };
            let dst = ml_metal_kernels::BufferOffset::zero_offset(&buffer);
            ml_metal_kernels::call_unary_strided(
                device.metal_device(),
                &encoder,
                device.kernels(),
                kernel_name,
        ...
        let new_storage = ml::MetalStorage::new(buffer, device.clone(), el_count, dtype);
...
pub fn dropout(xs: &Tensor, drop_p: f32) -> Result<Tensor> {
    ...
    if !(0. ..1.).contains(&drop_p) {
        ml::bail!("dropout probability has to be in [0, 1), got {drop_p}")
    }
...
impl ml::CustomOp1 for SoftmaxLastDim {
    fn name(&self) -> &'static str {
        "softmax-last-dim"
    }
    ...
    fn vulkan_fwd(
        &self,
        storage: &ml::VulkanStorage,
        layout: &Layout,
    ) -> Result<(ml::VulkanStorage, Shape)> {
        let v = storage.softmax_last_dim(layout)?;
        Ok((v, layout.shape().clone()))
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        fn softmax<T: ml::WithDType + num_traits::Float>(
            src: &[T],
            layout: &Layout,
        ) -> Result<(CpuStorage, Shape)> {
            let src = match layout.contiguous_offsets() {
                None => ml::bail!("input has to be contiguous"),
            ...
            let storage = ml::WithDType::to_cpu_storage_owned(dst);
        ...
        match storage {
            ...
            _ => ml::bail!("unsupported dtype for softmax {:?}", storage),
        }
    }
    ...
    fn cuda_fwd(
        ...
        impl Map1 for S {
            fn f<T: DeviceRepr + WithDType>(
                ...
                builder.arg(&src);
                builder.arg(&dst);
                ml::builder_arg!(builder, n_cols as i32);
                // SAFETY: ffi.
                unsafe { builder.launch(cfg) }.w()?;
                Ok(dst)
            }
        ...
        let dev = storage.device();
        let slice = S.map(&storage.slice, dev, layout)?;
        let dst = ml::cuda_backend::CudaStorage {
            slice,
            device: dev.clone(),
        };
        Ok((dst, layout.shape().clone()))
    ...
    fn metal_fwd(
        ...
        let name = match storage.dtype() {
            ...
            dtype => ml::bail!("softmax-last-dim is not implemented for {dtype:?}"),
        };

        let n = layout.stride().len();
        if !(layout.is_contiguous() && layout.stride()[n - 1] == 1) {
            ml::bail!("Non contiguous softmax-last-dim is not implemented");
        }

        let last_dim = layout.dims()[layout.shape().rank() - 1];
        let elem_count = layout.shape().elem_count();
        let output = device.new_buffer(elem_count, storage.dtype(), "softmax")?;
        ml_metal_kernels::call_last_softmax(
            device.metal_device(),
            &encoder,
            kernels,
            name,
        ...
        .map_err(ml::Error::wrap)?;
        let newstorage =
            ml::MetalStorage::new(output, device.clone(), elem_count, storage.dtype());
...
impl ml::CustomOp2 for RmsNorm {
    ...
    fn vulkan_fwd(
        ...
        let v = s1.rms_norm(l1, s2, l2, self.eps)?;
        Ok((v, l1.shape().clone()))
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let alpha = match alpha_layout.contiguous_offsets() {
                None => ml::bail!("alpha has to be contiguous"),
        ...
        match (s1, s2) {
            ...
            _ => ml::bail!("unsupported dtype for rmsnorm {:?}", s1.dtype()),
        }
    }
    ...
    fn cuda_fwd(
        ...
        use ml::cuda_backend::{kernel_name, kernels, Map2, WrapErr};
        use ml::{CudaDevice, WithDType};
        ...
        impl Map2 for S {
            fn f<T: DeviceRepr + WithDType>(
                ...
                builder.arg(&dst);
                builder.arg(&alpha);
                ml::builder_arg!(builder, n_cols as i32, block_size as i32, self.eps);
                // SAFETY: ffi.
                unsafe { builder.launch(cfg) }.w()?;
                Ok(dst)
            }
    ...
    fn metal_fwd(
        ...
        let name = match (s1.dtype(), s2.dtype()) {
            ...
            (dt1, dt2) => ml::bail!("rmsnorm is not implemented for {dt1:?} {dt2:?}"),
        };

        if !(l1.is_contiguous() && l2.is_contiguous()) {
            ml::bail!("Non contiguous rmsnorm is not implemented");
        }

        let last_dim = l1.dims()[l1.shape().rank() - 1];
        let elem_count = l1.shape().elem_count();
        let output = device.new_buffer(elem_count, s1.dtype(), "rmsnorm")?;
        ml_metal_kernels::call_rms_norm(
            device.metal_device(),
            &encoder,
            kernels,
            name,
        ...
        .map_err(ml::Error::wrap)?;
        let newstorage = ml::MetalStorage::new(output, device.clone(), elem_count, s1.dtype());
...
impl ml::CustomOp3 for LayerNorm {
    fn name(&self) -> &'static str {
        "layer-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let beta = match beta_layout.contiguous_offsets() {
                None => ml::bail!("beta has to be contiguous"),
    ...
    fn cuda_fwd(
        ...
        use ml::cuda_backend::{kernel_name, kernels, Map3, WrapErr};
        use ml::{CudaDevice, WithDType};
    ...
    fn metal_fwd(
        ...
        let name = match (s1.dtype(), s2.dtype(), s3.dtype()) {
            ...
            (dt1, dt2, dt3) => {
                ml::bail!("layernorm is not implemented for {dt1:?} {dt2:?} {dt3:?}")
            }
        };

        if !(l1.is_contiguous() && l2.is_contiguous() && l3.is_contiguous()) {
            ml::bail!("Non contiguous layernorm is not implemented");
        }

        let last_dim = l1.dims()[l1.shape().rank() - 1];
        let elem_count = l1.shape().elem_count();
        let output = device.new_buffer(elem_count, s1.dtype(), "layernorm")?;
        ml_metal_kernels::call_layer_norm(
            device.metal_device(),
            &encoder,
            kernels,
            name,
...
pub fn replication_pad2d(xs: &Tensor, pad: usize) -> Result<Tensor> {
    match pad {
        ...
        n => ml::bail!("replication-pad with a size of {n} is not supported"),
    }
}
...
impl ml::CustomOp3 for Sdpa {
    ...
    fn metal_fwd(
        ...
        use ml::backend::BackendStorage;
        use ml_metal_kernels::SdpaDType;
        ...
        if q_l.dim(D::Minus1)? != k_l.dim(D::Minus1)? {
            ml::bail!("`q` and `k` last dims must match");
        }

        // k,v must have matching n kv heads
        if v_l.dim(D::Minus(3))? != k_l.dim(D::Minus(3))? {
            ml::bail!("`k` and `v` head dims must match");
        }

        // n_heads % n_kv_heads == 0; n_heads >= 1, n_kv_heads >= 1.
        if q_l.dim(D::Minus(3))? % k_l.dim(D::Minus(3))? != 0 {
            ml::bail!("query `n_heads` must be a multiple of `n_kv_heads`");
        }
        ...
        for t in [k.dtype(), v.dtype()] {
            if q.dtype() != t {
                ml::bail!("all q, k, v dtypes must match.");
            }
        }
        ...
        let itype = match q.dtype() {
            ...
            other => ml::bail!("unsupported sdpa type {other:?}"),
        };
        ...
        if supports_sdpa_vector {
            ...
            if k_seq >= TWO_PASS_K_THRESHOLD {
                let mut intermediate_shape = [
                    ...
                    &[ml_metal_kernels::SDPA_2PASS_BLOCKS],
                ...
                ml_metal_kernels::call_sdpa_vector_2pass(
                    q.device().device(),
                    &encoder,
                    q.device().kernels(),
                    q_l.start_offset(),
                ...
                encoder.set_label("vector_attention");
                ml_metal_kernels::call_sdpa_vector(
                    q.device().device(),
                    &encoder,
                    q.device().kernels(),
                    q_l.start_offset(),
            ...
            if self.softcapping != 1. {
                ml::bail!("SDPA full requires softcapping to be disabled (1.0)");
            }
            ...
            let (mask_type, mask_buffer, mask_strides) = if let Some(mask) = &self.mask {
                ...
                let mask_buffer = match &**mask_s {
                    ml::Storage::Metal(m) => m.buffer(),
                    _ => ml::bail!("Expected metal device for mask"),
                };
                ...
                if mask_type != itype {
                    ml::bail!("Mask type {mask_type:?} must match q type {itype:?}");
                }
            ...
            ml_metal_kernels::call_sdpa_full(
                q.device().device(),
                &encoder,
                q.device().kernels(),
                q_l.start_offset(),
            ...
            ml::bail!("must be vector or full sdpa kernel");
        }

        let newstorage = ml::MetalStorage::new(output, device.clone(), elem_count, q.dtype());
