//! Rotary Embeddings
//!
use ml::{CpuStorage, Layout, Result, Shape, Tensor, D};
use rayon::prelude::*;
...
impl ml::CustomOp3 for RotaryEmbI {
    ...
    fn cpu_fwd(
        ...
        fn inner<T: ml::WithDType + num_traits::Float>(
            src: &[T],
            l_src: &Layout,
            cos: &[T],
            l_cos: &Layout,
            ...
            let src = match l_src.contiguous_offsets() {
                None => ml::bail!("input src has to be contiguous"),
                Some((o1, o2)) => &src[o1..o2],
            };
            let cos = match l_cos.contiguous_offsets() {
                None => ml::bail!("input cos has to be contiguous"),
                Some((o1, o2)) => &cos[o1..o2],
            };
            let sin = match l_sin.contiguous_offsets() {
                None => ml::bail!("input sin has to be contiguous"),
    ...
    fn cuda_fwd(
        ...
        use ml::cuda_backend::{kernel_name, kernels, WrapErr};
        ...
        fn inner<T: DeviceRepr + WithDType>(
            ...
            let src = match l_src.contiguous_offsets() {
                None => ml::bail!("src input has to be contiguous"),
                Some((o1, o2)) => src.slice(o1..o2),
            };
            let cos = match l_cos.contiguous_offsets() {
                None => ml::bail!("cos input has to be contiguous"),
                Some((o1, o2)) => cos.slice(o1..o2),
            };
            let sin = match l_sin.contiguous_offsets() {
                None => ml::bail!("sin input has to be contiguous"),
            ...
            builder.arg(&sin);
            builder.arg(&dst);
            ml::builder_arg!(builder, (b * h) as u32, (t * d) as u32, stride_b);
            // SAFETY: ffi.
        ...
        use ml::backend::BackendStorage;
        use ml::cuda_backend::CudaStorageSlice::{BF16, F16, F32, F64};
        let dev = s1.device();
    ...
    fn metal_fwd(
        ...
        let name = match src.dtype() {
            ml::DType::F32 => "rope_i_f32",
            ml::DType::F16 => "rope_i_f16",
            ml::DType::BF16 => "rope_i_bf16",
            dtype => ml::bail!("rope-i is not implemented for {dtype:?}"),
        };
        let (b, h, t, d) = l_src.shape().dims4()?;
        let stride_b = if l_cos.dims().len() == 3 && l_sin.dims().len() == 3 {
            h * t * d
        ...
        let out = ml::MetalStorage::new(output, device.clone(), el, src.dtype());
...
fn rope_check_cs(cs: &Tensor, b_sz: usize) -> Result<(usize, usize)> {
    match *cs.dims() {
        ...
        [b, t, d] => {
            if b != b_sz {
                ml::bail!("inconsistent batch size in rope {b_sz} {cs:?}",)
            }
            Ok((t, d))
        }
        _ => ml::bail!("cos/sin has to be 2D or 3D in rope {b_sz} {cs:?}"),
    }
}
...
pub fn rope_i(xs: &Tensor, cos: &Tensor, sin: &Tensor) -> Result<Tensor> {
    ...
    if !xs.is_contiguous() {
        ml::bail!("xs has to be contiguous in rope")
    }
    if !cos.is_contiguous() {
        ml::bail!("cos has to be contiguous in rope")
    }
    if !sin.is_contiguous() {
        ml::bail!("sin has to be contiguous in rope")
    }
...
impl ml::CustomOp3 for RotaryEmb {
    fn name(&self) -> &'static str {
        "rotary-emb"
    }
    ...
    fn cuda_fwd(
        ...
        fn inner<T: DeviceRepr + WithDType>(
            ...
            builder.arg(&sin);
            builder.arg(&dst);
            ml::builder_arg!(builder, (b * h) as u32, (t * d) as u32, d as u32, stride_b);
            // SAFETY: ffi.
            unsafe { builder.launch(cfg) }.w()?;
            Ok(dst)
        }
    ...
    fn metal_fwd(
        ...
        let name = match src.dtype() {
            ml::DType::F32 => "rope_f32",
            ml::DType::F16 => "rope_f16",
            ml::DType::BF16 => "rope_bf16",
            dtype => ml::bail!("rope is not implemented for {dtype:?}"),
        };
        let (b, h, t, d) = l_src.shape().dims4()?;
        let stride_b = if l_cos.dims().len() == 3 && l_sin.dims().len() == 3 {
            h * t * d
...
impl ml::CustomOp3 for RotaryEmbThd {
    fn name(&self) -> &'static str {
        "rotary-emb"
    }
    ...
    fn cuda_fwd(
        ...
        fn inner<T: DeviceRepr + WithDType>(
            ...
            builder.arg(&sin);
            builder.arg(&dst);
            ml::builder_arg!(builder, b as u32, t as u32, h as u32, d as u32, stride_b);
            // SAFETY: ffi.
            unsafe { builder.launch(cfg) }.w()?;
            Ok(dst)
        }
    ...
    fn metal_fwd(
        ...
        let name = match src.dtype() {
            ml::DType::F32 => "rope_thd_f32",
            ml::DType::F16 => "rope_thd_f16",
            ml::DType::BF16 => "rope_thd_bf16",
            dtype => ml::bail!("rope_thd is not implemented for {dtype:?}"),
        };
        let (b, t, h, d) = l_src.shape().dims4()?;
        let stride_b = if l_cos.dims().len() == 3 && l_sin.dims().len() == 3 {
            h * t * d
        ...
        ml_metal_kernels::call_rope_thd(
            device.metal_device(),
            &encoder,
            kernels,
            name,
