...
use ml::{test_utils::to_vec2_round, DType, Device, Result, Tensor};
use ml_nn::RNN;
...
fn lstm() -> Result<()> {
    ...
    .into_iter()
    .collect();
    let vb = ml_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let lstm = ml_nn::lstm(2, 3, Default::default(), vb)?;
...
fn gru() -> Result<()> {
    ...
    .collect();
    let vb = ml_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let gru = ml_nn::gru(2, 3, Default::default(), vb)?;
