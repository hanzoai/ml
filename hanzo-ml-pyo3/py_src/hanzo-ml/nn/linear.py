...
class Linear(Module):
    ...
    def __init__(
        ...
        self.weight = hanzo.ones((out_features, in_features), **factory_kwargs)
        if bias:
            self.bias = hanzo.zeros((out_features,), **factory_kwargs)
        else:
            self.bias = None
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if isinstance(self.weight, hanzo.QTensor):
            if len(dims) < 3:
