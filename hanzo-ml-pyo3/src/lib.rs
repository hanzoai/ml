...
use ::hanzo_ml::{quantized::QTensor, DType, Device, Module, Tensor, WithDType};
...
fn actual_index(t: &Tensor, dim: usize, index: i64) -> ::hanzo_ml::Result<usize> {
    let dim = t.dim(dim)?;
    if 0 <= index {
        let index = index as usize;
        if dim <= index {
            ::hanzo_ml::bail!("index {index} is too large for tensor dimension {dim}")
        }
        Ok(index)
    } else {
        if (dim as i64) < -index {
            ::hanzo_ml::bail!("index {index} is too low for tensor dimension {dim}")
        }
        Ok((dim as i64 + index) as usize)
    }
}

fn actual_dim(t: &Tensor, dim: i64) -> ::hanzo_ml::Result<usize> {
    let rank = t.rank();
    if 0 <= dim {
        let dim = dim as usize;
        if rank <= dim {
            ::hanzo_ml::bail!("dimension index {dim} is too large for tensor rank {rank}")
        }
        Ok(dim)
    } else {
        if (rank as i64) < -dim {
            ::hanzo_ml::bail!("dimension index {dim} is too low for tensor rank {rank}")
        }
        Ok((rank as i64 + dim) as usize)
    }
}

// TODO: Something similar to this should probably be a part of hanzo core.
trait MapDType {
    type Output;
...
impl PyTensor {
    ...
    /// Converts hanzo's tensor to pytorch's tensor
    ...
    fn __richcmp__(&self, rhs: &Bound<PyAny>, op: CompareOp) -> PyResult<Self> {
        ...
        if let Ok(rhs) = rhs.extract::<PyTensor>() {
            if self.0.shape() == rhs.0.shape() {
                ...
                // We broadcast manually here because `hanzo.cmp` does not support automatic broadcasting
...
impl PyQTensor {
    ...
    fn matmul_t(&self, lhs: &PyTensor) -> PyResult<PyTensor> {
        let qmatmul = ::hanzo_ml::quantized::QMatMul::from_arc(self.0.clone()).map_err(wrap_err)?;
...
fn save_safetensors(
    ...
    ::hanzo_ml::safetensors::save(&tensors, path).map_err(wrap_err)
}
...
fn load_ggml(
    ...
    let device = device.unwrap_or(PyDevice::Cpu).as_device()?;
    let ggml =
        ::hanzo_ml::quantized::ggml_file::Content::read(&mut file, &device).map_err(wrap_err)?;
    let tensors = ggml
        .tensors
        .into_iter()
...
fn cuda_is_available() -> bool {
    ::hanzo_ml::utils::cuda_is_available()
}
...
fn get_num_threads() -> usize {
    ::hanzo_ml::utils::get_num_threads()
}
...
fn hanzo_functional_m(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
#[cfg(feature = "onnx")]
fn hanzo_onnx_m(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
