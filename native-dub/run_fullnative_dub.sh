...
SRC_VIDEO="${SRC_VIDEO:-$RUN/clip/sun6.mp4}"        # video-only talking head
SRC_AUDIO_ZH="${SRC_AUDIO_ZH:-$RUN/clip/orig6_16k.wav}"  # source speech, 16k mono
...
QWEN_GGUF="${QWEN_GGUF:-$HOME/work/zen/hf/Qwen3-8B-GGUF/Qwen3-8B-UD-IQ2_M.gguf}"
...
echo "[1/6] zen3-ASR transcribe (zh)"
TRANSCRIPT_ZH=$("$ZEN3" asr --model "$ASR_MODEL" --audio "$SRC_AUDIO_ZH" --max-new 128 2>/dev/null | tail -1)
echo "    zh: $TRANSCRIPT_ZH"

echo "[2/6] Qwen3 translate zh->en (native hanzo engine)"
PROMPT="Translate this Chinese sentence to natural English. Output only the translation.\n\n$TRANSCRIPT_ZH"
TRANSLATION_EN=$("$HANZO" run --format gguf -m "$QWEN_GGUF" -i "$PROMPT" 2>/dev/null | tail -1)
echo "    en: $TRANSLATION_EN"

echo "[3/6] zen3-TTS synthesize en (sampling; greedy collapses to silence)"
"$ZEN3" tts --model "$TTS_MODEL" --text "$TRANSLATION_EN" --out "$WORK/tts_en.wav" --max-tokens 1200
