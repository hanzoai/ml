...
/// Tile configuration for the GEMM kernel.
///
...
// All of these are instantiated in metal_src/mlx_gemm.metal.
#[allow(dead_code)]
...
/// (mlx/backend/metal/matmul.cpp lines 88-170).
///
...
fn select_tile_config(
    ...
    // Special case: For very small M (vector-matrix multiply), use the original
    // 32x32 tile to avoid thread waste. When M is very small (< bm), larger bm
    // values cause significant thread underutilization in the M dimension.
    // Critical for benchmarks like [1, 2048] @ [2048, 2048] (m=1).
    if m < 16 {
    ...
    match device_type {
        ...
        MetalDeviceType::Max | MetalDeviceType::Medium => match dtype {
            GemmDType::F32 => {
                if !is_large_matmul {
                    if !a_trans && b_trans {
                        TILE_32_64_16_1_2
...
///
/// MLX's batch collapse optimization (matmul.cpp lines 700-740): when B is
/// broadcasted (2D) we can collapse batch into M:
/// - [batch, M, K] @ [K, N] -> [batch*M, K] @ [K, N]
///
/// Conditions:
/// 1. batch_size > 1
/// 2. !transpose_a (A row-major for M dimension)
/// 3. A contiguous in batch (batch_stride_a == M * K)
/// 4. B broadcasted (batch_stride_b == 0, i.e. B is 2D)
///
...
fn check_batch_collapse(
    ...
    // A's batch stride must be contiguous (batch_stride_a == M * K).
    ...
    // B's batch stride must be 0 (broadcasted) for collapse.
...
/// Check if we can use a split-K strategy for better performance.
///
/// MLX uses split-K when batch_size == 1, (M/16)*(N/16) <= 32 (small output),
/// and K/16 >= 8 (large K). Useful for tall-skinny matrices where K >> M*N.
#[allow(dead_code)]
...
pub fn call_mlx_gemv(
    ...
    encoder.use_resource(mat_buffer, MTLResourceUsage::Read);
    encoder.use_resource(vec_buffer, MTLResourceUsage::Read);
...
pub fn call_mlx_gemm(
    ...
    // Vector-matrix / matrix-vector products dispatch to the dedicated gemv kernels.
    if m == 1 || n == 1 {
    ...
    // Batch strides for buffer 7.
    let batch_strides = [batch_stride_a, batch_stride_b];

    set_params!(
        encoder,
        (
            Input::with_offset(lhs_buffer, lhs_offset),
            Input::with_offset(rhs_buffer, rhs_offset),
            (),
            Output::new(output),
            gemm_params,
            (),
