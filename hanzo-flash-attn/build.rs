...
fn main() -> Result<()> {
    ...
    // Kernel objects cached in FLASH_ATTN_BUILD_DIR must rebuild when a shared header changes.
    let header_hash_arg: &'static str =
        Box::leak(format!("-DFLASH_ATTN_HEADER_HASH=0x{:016x}", header_hash()?).into_boxed_str());
