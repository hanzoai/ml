...
impl CustomOp1 for NonZero {
    ...
    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        ...
        let result = match storage {
            ...
            hanzo::CpuStorage::I16(vs) => self.nonzero(vs, layout),
            hanzo::CpuStorage::I32(vs) => self.nonzero(vs, layout),
            ...
            hanzo::CpuStorage::F8E4M3(vs) => self.nonzero(vs, layout),
            // Dummy types don't support nonzero operation
            hanzo::CpuStorage::F6E2M3(_) => {
                return Err(
                    hanzo::Error::UnsupportedDTypeForOp(hanzo::DType::F6E2M3, "nonzero").bt(),
                )
            }
            hanzo::CpuStorage::F6E3M2(_) => {
                return Err(
                    hanzo::Error::UnsupportedDTypeForOp(hanzo::DType::F6E3M2, "nonzero").bt(),
                )
            }
            hanzo::CpuStorage::F4(_) => {
                return Err(hanzo::Error::UnsupportedDTypeForOp(hanzo::DType::F4, "nonzero").bt())
            }
            hanzo::CpuStorage::F8E8M0(_) => {
                return Err(
                    hanzo::Error::UnsupportedDTypeForOp(hanzo::DType::F8E8M0, "nonzero").bt(),
                )
            }
        };
