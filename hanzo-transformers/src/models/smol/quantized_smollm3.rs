    ...
    pub fn from_gguf(ct: &gguf_file::Content) -> Result<Self> {
        ...
        let get_u32 = |key: &str| -> Result<usize> {
            metadata
                ...
                .map(|v| v as usize)
                .ok_or_else(|| {
                    hanzo::Error::Msg(format!("Missing or invalid metadata key: {}", key))
                })
        };
...
impl RmsNorm {
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let hidden_size = x.dim(hanzo::D::Minus1)?;
        let x = x.to_dtype(internal_dtype)?;
        let norm_x = (x.sqr()?.sum_keepdim(hanzo::D::Minus1)? / hidden_size as f64)?;
...
impl QuantizedAttention {
    fn new(
        ...
        use hanzo::quantized::{GgmlDType, QTensor};
    ...
    fn forward(&mut self, x: &Tensor, mask: Option<&Tensor>, offset: usize) -> Result<Tensor> {
        ...
        // can remove this continguous call if using ConcatKV-Cache https://github.com/huggingface/hanzo/pull/3143
...
impl QuantizedModelForCausalLM {
    pub fn from_gguf<P: AsRef<std::path::Path>>(path: P, device: &Device) -> Result<Self> {
        ...
        let embed_tokens = hanzo_nn::Embedding::new(embed_tensor_gpu, config.hidden_size);
