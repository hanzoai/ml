    ...
    pub fn from_gguf(ct: &gguf_file::Content) -> Result<Self> {
        ...
        let get_u32 = |key: &str| -> Result<usize> {
            metadata
                ...
                .map(|v| v as usize)
                .ok_or_else(|| {
                    hanzo_ml::Error::Msg(format!("Missing or invalid metadata key: {}", key))
                })
        };
...
impl RmsNorm {
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let hidden_size = x.dim(hanzo_ml::D::Minus1)?;
        let x = x.to_dtype(internal_dtype)?;
        let norm_x = (x.sqr()?.sum_keepdim(hanzo_ml::D::Minus1)? / hidden_size as f64)?;
