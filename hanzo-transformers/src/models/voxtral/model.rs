...
pub fn find_audio_token_positions(
    ...
    let input_ids = if input_ids.dtype() == hanzo::DType::U32 {
        input_ids.to_dtype(hanzo::DType::I64)?
    } else {
        input_ids.clone()
    };
...
impl VoxtralEncoder {
    ...
    pub fn forward_with_training(&self, input_features: &Tensor, training: bool) -> Result<Tensor> {
        ...
        let x = if false {
            ...
            let x_f32 = x.to_dtype(hanzo::DType::F32)?;
            let positions_f32 = positions.to_dtype(hanzo::DType::F32)?;
...
impl VoxtralForConditionalGeneration {
    ...
    pub fn get_audio_embeds(&self, input_features: &Tensor) -> Result<Tensor> {
        ...
        if total_elements % self.audio_config.intermediate_size != 0 {
            return Err(hanzo::Error::DimOutOfRange {
                shape: hanzo::Shape::from_dims(&[batch_size, seq_len, hidden_size]),
                dim: 0,
                op: "reshape",
            });
        }
    ...
    pub fn generate(
        ...
        if let Some(p) = config.top_p {
            if !(0.0..=1.0).contains(&p) {
                hanzo::bail!("top_p must be between 0 and 1, got {}", p);
            }
        }
        ...
        for idx in 0..config.max_new_tokens {
            ...
            let next_token = if config.temperature > 0.0 {
                ...
                let argmax_result = match logits.argmax(D::Minus1) {
                    Ok(result) => result,
                    Err(e) => {
                        return Err(hanzo::Error::Msg(format!("Argmax failed: {e}")));
                    }
                };
                ...
                if argmax_result.dims().is_empty() {
                    ...
                    match argmax_result.to_scalar::<u32>() {
                        Ok(token) => token,
                        Err(e) => {
                            return Err(hanzo::Error::Msg(format!("to_scalar failed: {e}")));
                        }
                    }
