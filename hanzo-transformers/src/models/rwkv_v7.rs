...
fn layer_norm(xs: &Tensor, weight: &Tensor, bias: &Tensor, eps: f64) -> Result<Tensor> {
    ...
    let dim = xs_f32.dim(hanzo::D::Minus1)?;
    let mean = (xs_f32.sum_keepdim(hanzo::D::Minus1)? / dim as f64)?;
    let centered = xs_f32.broadcast_sub(&mean)?;
    let var = (centered.sqr()?.sum_keepdim(hanzo::D::Minus1)? / dim as f64)?;
...
impl Model {
    ...
    pub fn forward_seq(&self, token_ids: &[u32], state: &mut State) -> Result<Tensor> {
        if token_ids.is_empty() {
            hanzo::bail!("token_ids cannot be empty");
        }
        ...
        last_logits.ok_or_else(|| hanzo::Error::Msg("No tokens processed".to_string()))
    }
}
