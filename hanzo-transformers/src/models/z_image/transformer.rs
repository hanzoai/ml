...
use hanzo_nn::{linear, linear_no_bias, VarBuilder};
...
fn flash_attn(_: &Tensor, _: &Tensor, _: &Tensor, _: f32, _: bool) -> Result<Tensor> {
    hanzo_ml::bail!("flash-attn feature not enabled, compile with '--features flash-attn'")
}
...
pub struct TimestepEmbedder {
    linear1: hanzo_nn::Linear,
    linear2: hanzo_nn::Linear,
...
pub struct ZImageAttention {
    to_q: hanzo_nn::Linear,
    to_k: hanzo_nn::Linear,
    to_v: hanzo_nn::Linear,
    to_out: hanzo_nn::Linear,
...
impl ZImageAttention {
    ...
    fn attention_metal(
        ...
        hanzo_nn::ops::sdpa(q, k, v, sdpa_mask.as_ref(), false, scale as f32, 1.0)
    }
...
pub struct ZImageTransformerBlock {
    ...
    ffn_norm1: RmsNorm,
    ffn_norm2: RmsNorm,
    adaln_modulation: Option<hanzo_nn::Linear>,
}
...
pub struct FinalLayer {
    ...
    linear: hanzo_nn::Linear,
    adaln_silu: hanzo_nn::Linear,
}

impl FinalLayer {
    pub fn new(hidden_size: usize, out_channels: usize, vb: VarBuilder) -> Result<Self> {
        let norm_final = LayerNormNoParams::new(1e-6);
        let linear = hanzo_nn::linear(hidden_size, out_channels, vb.pp("linear"))?;
        let adaln_dim = hidden_size.min(ADALN_EMBED_DIM);
        let adaln_silu =
            hanzo_nn::linear(adaln_dim, hidden_size, vb.pp("adaLN_modulation").pp("1"))?;
...
pub struct ZImageTransformer2DModel {
    ...
    cap_embedder_linear: hanzo_nn::Linear,
    x_embedder: hanzo_nn::Linear,
