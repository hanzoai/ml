...
impl NormConv1d {
    ...
    pub fn new(
        ...
        let conv = match norm {
            ...
            Some(Norm::SpectralNorm) => hanzo_ml::bail!("SpectralNorm is not supported yet."),
        };
        let norm = match norm {
...
fn unpad1d(xs: &Tensor, unpad_l: usize, unpad_r: usize) -> Result<Tensor> {
    ...
    if len < unpad_l + unpad_r {
        hanzo_ml::bail!("unpad1d: tensor len {len} is too low, {unpad_l} + {unpad_r}")
    }
...
impl StreamableConv1d {
    ...
    pub fn new(
        ...
        if k_size < stride {
            hanzo_ml::bail!("kernel-size {k_size} is smaller than stride {stride}")
        }
        Ok(Self {
            conv,
            causal,
...
impl ConvDownsample1d {
    pub fn new(
        ...
        if !learnt {
            hanzo_ml::bail!("only learnt=true is supported")
        }
...
mod tests {
    ...
    fn run_conv1d(
        ...
        if diff > 1e-5 {
            ...
            println!("{ys}");
            println!("{ys_steps}");
            hanzo_ml::bail!("larger diff than expected {diff}")
        }
        Ok(())
    }
