...
use hanzo_ml::Module;
use hanzo_ml::{quantized::QTensor, DType, Result, Tensor, D};
use hanzo_nn::{linear_no_bias, moe, Activation, Linear, VarBuilder};
use std::sync::Arc;
...
impl FusedMoe {
    ...
    pub fn forward(&self, xs: &Tensor, is_prefill: bool) -> Result<Tensor> {
        ...
        let gate = gate_up
            .narrow(hanzo_ml::D::Minus1, 0, self.w_size_n)?
            .contiguous()?;
        let up = gate_up
            .narrow(hanzo_ml::D::Minus1, self.w_size_n, self.w_size_n)?
            .contiguous()?;
