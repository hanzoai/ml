...
use ml_transformers::models::{clip, flux, t5};

use anyhow::{Error as E, Result};
use ml::{IndexOp, Module, Tensor};
...
fn run(args: Args) -> Result<()> {
    ...
    let img = match decode_only {
            ...
            let img = {
                ...
                let state = if quantized {
                    flux::sampling::State::new(
                        &t5_emb.to_dtype(ml::DType::F32)?,
                        &clip_emb.to_dtype(ml::DType::F32)?,
                        &img.to_dtype(ml::DType::F32)?,
                    )?
                } else {
        ...
        Some(file) => {
            let mut st = ml::safetensors::load(file, &device)?;
    ...
    println!("img\n{img}");
    let img = ((img.clamp(-1f32, 1f32)? + 1.0)? * 127.5)?.to_dtype(ml::DType::U8)?;
...
fn main() -> Result<()> {
    let args = Args::parse();
    #[cfg(feature = "cuda")]
    ml::quantized::cuda::set_force_dmmv(args.use_dmmv);
    run(args)
}
