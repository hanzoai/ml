...
use ml_transformers::object_detection::{non_maximum_suppression, Bbox, KeyPoint};
...
pub fn report_detect(
    ...
    let font = ab_glyph::FontRef::try_from_slice(&font).map_err(ml::Error::wrap)?;
...
pub fn report_pose(
    ...
    if pred_size != 17 * 3 + 4 + 1 {
        ml::bail!("unexpected pred-size {pred_size}");
    }
    let mut bboxes = vec![];
    // Extract the bounding boxes for which confidence is above the threshold.
    for index in 0..npreds {
