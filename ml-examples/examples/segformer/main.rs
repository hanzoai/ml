...
use ml_transformers::models::segformer::{
...
fn get_vb_and_config(
    ...
    let vb =
        unsafe { VarBuilder::from_mmaped_safetensors(&[model_file], ml::DType::F32, device)? };
...
fn classification_task(args: ClassificationArgs, device: &Device) -> anyhow::Result<()> {
    ...
    let classification = ml_nn::ops::softmax_last_dim(&classification)?;
