...
pub fn main() -> Result<()> {
    ...
    match args.command {
        Command::Print { file } => {
            let model = ml_onnx::read_file(file)?;
            println!("{model:?}");
        ...
        Command::SimpleEval { file } => {
            ...
            for input in graph.input.iter() {
                ...
                let value = match type_ {
                    ml_onnx::onnx::type_proto::Value::TensorType(tt) => {
                        let dt = match DataType::try_from(tt.elem_type) {
                            Ok(dt) => match ml_onnx::dtype(dt) {
                                Some(dt) => dt,
                                None => {
                                    anyhow::bail!(
                        ...
                        let dims = shape
                                ...
                                .map(|dim| match dim.value.as_ref().expect("no dim value") {
                                    ml_onnx::onnx::tensor_shape_proto::dimension::Value::DimValue(v) => Ok(*v as usize),
                                    ml_onnx::onnx::tensor_shape_proto::dimension::Value::DimParam(_) => Ok(42),
                                })
            ...
            let outputs = ml_onnx::simple_eval(&model, inputs)?;
