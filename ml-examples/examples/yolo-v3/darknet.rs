use ml::{DType, Device, IndexOp, Result, Tensor};
use ml_nn::{batch_norm, conv2d, conv2d_no_bias, Func, Module, VarBuilder};
...
impl Block {
    fn get(&self, key: &str) -> Result<&str> {
        match self.parameters.get(key) {
            None => ml::bail!("cannot find {} in {}", key, self.block_type),
            Some(value) => Ok(value),
        }
    }
}
...
impl Darknet {
    fn get(&self, key: &str) -> Result<&str> {
        match self.parameters.get(key) {
            None => ml::bail!("cannot find {} in net parameters", key),
            Some(value) => Ok(value),
        }
    }
}
...
pub fn parse_config<T: AsRef<Path>>(path: T) -> Result<Darknet> {
    ...
    for line in BufReader::new(file).lines() {
        ...
        if line.starts_with('[') {
            if !line.ends_with(']') {
                ml::bail!("line does not end with ']' {line}")
            }
            ...
            if prev.is_some() {
                ml::bail!("multiple value for key {}", line)
            }
        }
    }
    acc.finish_block();
...
enum Bl {
    Layer(Box<dyn ml_nn::Module + Send + Sync>),
    Route(Vec<usize>),
    Shortcut(usize),
...
fn conv(vb: VarBuilder, index: usize, p: usize, b: &Block) -> Result<(usize, Bl)> {
    ...
    let conv_cfg = ml_nn::Conv2dConfig {
        stride,
        padding,
        groups: 1,
        dilation: 1,
    ...
    let leaky = match activation {
        "leaky" => true,
        "linear" => false,
        otherwise => ml::bail!("unsupported activation {}", otherwise),
    };
    let func = ml_nn::func(move |xs| {
...
fn yolo(p: usize, block: &Block) -> Result<(usize, Bl)> {
    ...
    if flat.len() % 2 != 0 {
        ml::bail!("even number of anchors");
    }
...
fn detect(
    ...
    let ys24 = xs.i((.., .., 2..4))?;
    let ys4 = xs.i((.., .., 4..))?;
    let ys02 = (ml_nn::ops::sigmoid(&ys02)?.add(&xy_offset)? * stride as f64)?;
    let ys24 = (ys24.exp()?.mul(&anchors)? * stride as f64)?;
    let ys4 = ml_nn::ops::sigmoid(&ys4)?;
    ...
    pub fn build_model(&self, vb: VarBuilder) -> Result<Func<'_>> {
        ...
        for (index, block) in self.blocks.iter().enumerate() {
            let channels_and_bl = match block.block_type.as_str() {
                ...
                otherwise => ml::bail!("unsupported block type {}", otherwise),
            };
