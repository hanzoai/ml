...
use ml_transformers::models::llama2_c as model;
use ml_transformers::models::llama2_c_weights as weights;
use ml_transformers::models::quantized_llama2_c as qmodel;
mod training;
use clap::{Parser, Subcommand};

use anyhow::{Error as E, Result};
use byteorder::{LittleEndian, ReadBytesExt};
use ml::{IndexOp, Tensor};
...
fn run_inference(args: &InferenceCmd, common_args: &Args) -> Result<()> {
    ...
    let device = candle_examples::device(common_args.cpu)?;
    #[cfg(feature = "cuda")]
    if let ml::Device::Cuda(d) = &device {
        unsafe {
    ...
    let (model, config, mut cache) = if is_gguf {
        ...
        let fake_vb = ml_nn::VarBuilder::from_tensors(
            [
        ...
        let tensors = ml::safetensors::load(config_path, &device)?;
        let vb = ml_nn::VarBuilder::from_tensors(tensors, ml::DType::F32, &device);
