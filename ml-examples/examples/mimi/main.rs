...
use ml_transformers::models::mimi::{Config, Model};
...
fn main() -> Result<()> {
    ...
    let codes = match args.action {
        Action::CodeToAudio => {
            let codes = ml::safetensors::load(args.in_file, &device)?;
        ...
        Action::AudioToCode | Action::AudioToAudio => {
            ...
            match args.streaming {
                Some(chunk_size) => {
                    ...
                    Tensor::cat(&code_chunks, ml::D::Minus1)?
                }
                None => {
                    let pcm_len = pcm.len();
    ...
    match args.action {
        ...
        Action::AudioToAudio | Action::CodeToAudio => {
            let pcm = match args.streaming {
                Some(chunk_size) => {
                    let seq_len = codes.dim(ml::D::Minus1)?;
