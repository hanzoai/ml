...
use ml_transformers::models::encodec;
use ml_transformers::models::metavoice::{adapters, gpt, tokenizers, transformer};
use ml_transformers::models::quantized_metavoice::transformer as qtransformer;

use ml::{DType, IndexOp, Tensor};
...
fn main() -> Result<()> {
    ...
    let mut first_stage_model = if args.quantized {
        ...
        let vb =
            ml_transformers::quantized_var_builder::VarBuilder::from_gguf(filename, &device)?;
    ...
    let spk_emb = ml::safetensors::load(&spk_emb_file, &ml::Device::Cpu)?;
    ...
    for logits in logits.iter() {
        ...
        for step in 0..seq_len {
            ...
            let prs = ml_nn::ops::softmax_last_dim(logits)?.to_vec1::<f32>()?;
