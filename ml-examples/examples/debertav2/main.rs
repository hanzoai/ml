...
use ml::{Device, Tensor};
use ml_nn::ops::softmax;
use ml_nn::VarBuilder;
use ml_transformers::models::debertav2::{Config as DebertaV2Config, DebertaV2NERModel};
use ml_transformers::models::debertav2::{DebertaV2SeqClassificationModel, Id2Label};
use ml_transformers::models::debertav2::{NERItem, TextClassificationItem};
...
impl Args {
    fn build_model_and_tokenizer(
        ...
        let mut tokenizer = Tokenizer::from_file(tokenizer_filename)
            .map_err(|e| ml::Error::Msg(format!("Tokenizer error: {e}")))?;
        tokenizer.with_padding(Some(PaddingParams::default()));

        let vb = if self.use_pth {
            VarBuilder::from_pth(
                &weights_filename,
                ml_transformers::models::debertav2::DTYPE,
                &device,
            )?
        } else {
            unsafe {
...
) -> impl Fn(F) -> Result<(), ml::Error>
where
    F: Fn(&Tensor, Tensor, Tensor) -> Result<(), ml::Error>,
{
    move |code: F| -> Result<(), ml::Error> {
