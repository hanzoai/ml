...
use ml_transformers::models::csm::{Config, Model};
...
fn main() -> Result<()> {
    ...
    let mut mimi_model = {
        use ml_transformers::models::mimi;
        let vb =
    ...
    let voices = ml::safetensors::load(args.voices, &device)?;
    let mut lp = ml_transformers::generation::LogitsProcessor::new(
        args.seed,
