...
pub fn main() -> anyhow::Result<()> {
    ...
    let model = ml_onnx::read_file(model)?;
    let graph = model.graph.as_ref().unwrap();
    let mut inputs = std::collections::HashMap::new();
    inputs.insert(graph.input[0].name.to_string(), image.unsqueeze(0)?);
    let mut outputs = ml_onnx::simple_eval(&model, inputs)?;
    let output = outputs.remove(&graph.output[0].name).unwrap();
    let prs = match args.which {
        Which::SqueezeNet => ml_nn::ops::softmax(&output, D::Minus1)?,
    ...
    match args.which {
        ...
        Which::EsrGan => {
            let max_pixel_val = ml::Tensor::try_from(255.0f32)?
                .to_device(prs.device())?
                .broadcast_as(prs.shape())?;
            let out = (prs * max_pixel_val)?.i(0)?.to_dtype(ml::DType::U8)?;
