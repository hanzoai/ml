...
#### Missing protoc installation when compiling hanzo-ml-onnx

The hanzo-ml-onnx dependency prost-build no longer comes bundled with prost
binaries. This could cause the following error when attempting to compile
hanzo-ml-onnx:

```
error: failed to run custom build command for `hanzo-ml-onnx`
Caused by: // (...)
