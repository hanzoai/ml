...
Here, we provide two examples of how to run YOLOv8 using a Hanzo-compiled WASM binary and runtimes.

### Pure Rust UI

To build and test the UI made in Rust you will need [Trunk](https://trunkrs.dev/#install)
From the `hanzo-wasm-examples/yolo` directory run:

Download assets:

```bash
wget -c https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/hanzo/examples/bike.jpeg
wget -c https://huggingface.co/lmz/hanzo-yolo-v8/resolve/main/yolov8s.safetensors
```
