...
let weights = ml::safetensors::load(weights_filename, &Device::Cpu).unwrap();
...
let weights = ml::safetensors::load_buffer(&mmap[..], &Device::Cpu).unwrap();
...
let mnist = ml_datasets::vision::Dataset {
    train_images,
    train_labels,
    test_images,
    test_labels,
