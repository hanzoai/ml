...
use ml::{DType, Result, Tensor, D, Device};
use ml_nn::{loss, ops, Linear, Module, VarBuilder, VarMap, Optimizer};
...
impl MultiLevelPerceptron {
    fn new(vs: VarBuilder) -> Result<Self> {
        let ln1 = ml_nn::linear(VOTE_DIM, LAYER1_OUT_SIZE, vs.pp("ln1"))?;
        let ln2 = ml_nn::linear(LAYER1_OUT_SIZE, LAYER2_OUT_SIZE, vs.pp("ln2"))?;
        let ln3 = ml_nn::linear(LAYER2_OUT_SIZE, RESULTS + 1, vs.pp("ln3"))?;
        Ok(Self { ln1, ln2, ln3 })
    }
...
fn train(m: Dataset, dev: &Device) -> anyhow::Result<MultiLevelPerceptron> {
    ...
    let mut sgd = ml_nn::SGD::new(varmap.all_vars(), LEARNING_RATE)?;
