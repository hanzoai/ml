...
use hanzo_ml::{utils, DType, Device, Tensor};
use hanzo_nn::VarBuilder;
use hanzo_transformers::models::voxtral;
use hanzo_transformers::models::voxtral::{
...
impl VoxtralModel {
    ...
    pub fn transcribe_audio(
        ...
        let audio = if sample_rate == SAMPLE_RATE {
            ...
            hanzo_ml_examples::audio::resample(audio_data, sample_rate, SAMPLE_RATE)
...
fn load_model_weights<'a>(model_files: &'a [PathBuf], device: &Device) -> Result<VarBuilder<'a>> {
    ...
    if let hanzo_ml::Device::Cuda(_) = device {
        device.synchronize()?;
    }
