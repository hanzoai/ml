...
pub fn main() -> anyhow::Result<()> {
    ...
    let model = match args.model {
        ...
        None => match args.which {
            Which::SqueezeNet => hanzo_ml_examples::hub::Api::new()?
                .model("lmz/hanzo-onnx")
                .get("squeezenet1.1-7.onnx")?,
            Which::EfficientNet => hanzo_ml_examples::hub::Api::new()?
                .model("onnx/EfficientNet-Lite4")
                .get("efficientnet-lite4-11.onnx")?,
            Which::EsrGan => hanzo_ml_examples::hub::Api::new()?
                .model("qualcomm/Real-ESRGAN-x4plus")
                .get("Real-ESRGAN-x4plus.onnx")?,
        },
    };

    let model = hanzo_onnx::read_file(model)?;
    ...
    match args.which {
        ...
        Which::EsrGan => {
            let max_pixel_val = hanzo_ml::Tensor::try_from(255.0f32)?
                .to_device(prs.device())?
                .broadcast_as(prs.shape())?;
            let out = (prs * max_pixel_val)?.i(0)?.to_dtype(hanzo_ml::DType::U8)?;
