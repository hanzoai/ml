...
impl Block {
    fn get(&self, key: &str) -> Result<&str> {
        match self.parameters.get(key) {
            None => hanzo_ml::bail!("cannot find {} in {}", key, self.block_type),
            Some(value) => Ok(value),
        }
    }
}
...
impl Darknet {
    fn get(&self, key: &str) -> Result<&str> {
        match self.parameters.get(key) {
            None => hanzo_ml::bail!("cannot find {} in net parameters", key),
            Some(value) => Ok(value),
        }
    }
}
...
pub fn parse_config<T: AsRef<Path>>(path: T) -> Result<Darknet> {
    ...
    for line in BufReader::new(file).lines() {
        ...
        if line.starts_with('[') {
            if !line.ends_with(']') {
                hanzo_ml::bail!("line does not end with ']' {line}")
            }
            ...
            if key_value.len() != 2 {
                hanzo_ml::bail!("missing equal {line}")
            }
            ...
            if prev.is_some() {
                hanzo_ml::bail!("multiple value for key {}", line)
            }
        }
    }
    acc.finish_block();
...
fn yolo(p: usize, block: &Block) -> Result<(usize, Bl)> {
    ...
    if flat.len() % 2 != 0 {
        hanzo_ml::bail!("even number of anchors");
    }
...
impl Darknet {
    ...
    pub fn build_model(&self, vb: VarBuilder<'_>) -> Result<Func<'_>> {
        ...
        for (index, block) in self.blocks.iter().enumerate() {
            let channels_and_bl = match block.block_type.as_str() {
                ...
                otherwise => hanzo_ml::bail!("unsupported block type {}", otherwise),
            };
