...
fn ug_op() -> Result<()> {
    let kernel = {
        use hanzo_ug::lang::op;

        let layout = hanzo_ug::Layout::from_shape(&[12]);
        let ptr = op::Arg::ptr(hanzo_ug::DType::F32);
        let src = op::load(ptr.id(), layout.clone(), hanzo_ug::DType::F32)?;
        ...
        let opts: hanzo_ug::lower_op::Opts = Default::default();
        kernel.lower(&opts)?
    };
