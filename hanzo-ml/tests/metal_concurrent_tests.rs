...
// A readback must wait on the command buffer holding its OWN blit, not on shared device state: waiting
// on "the last in-flight buffer" lets a concurrent flush on another thread drain the buffer carrying
// this thread's work, so the wait returns before the blit has run and the CPU reads a stale buffer.
#[test]
