...
use hanzo_ml::cutile;
use hanzo_ml::{CpuStorage, CudaStorage, CustomOp2, Device, Layout, Result, Shape, Tensor};
use std::sync::Mutex;
...
impl CustomOp2 for Add {
    ...
    fn cpu_fwd(
        ...
        hanzo_ml::bail!("cutile-test-add requires CUDA")
    }
    ...
    fn cuda_fwd(
        ...
        if !lhs_layout.is_contiguous() || !rhs_layout.is_contiguous() {
            hanzo_ml::bail!("cutile-test-add requires contiguous tensors")
        }
        if lhs_layout.dims() != rhs_layout.dims() {
            hanzo_ml::bail!("cutile-test-add requires equal shapes")
        }

        let len = lhs_layout.shape().elem_count();
        if !len.is_multiple_of(BLOCK_SIZE) {
            hanzo_ml::bail!("cutile-test-add requires a multiple of {BLOCK_SIZE} elements")
        }
...
fn cuda_device() -> Result<Option<Device>> {
    if hanzo_ml::utils::cuda_is_available() {
...
fn compile_only_warmup_does_not_launch() -> Result<()> {
    ...
    let rhs =
        Tensor::ones(WARMUP_SIZE + 1, hanzo_ml::DType::F32, &device)?.narrow(0, 1, WARMUP_SIZE)?;
...
#[test]
fn custom_op_preserves_hanzo_stream_ordering() -> Result<()> {
    let _lock = GPU_TEST_LOCK
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let Some(device) = cuda_device()? else {
