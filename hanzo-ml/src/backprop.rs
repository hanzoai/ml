            ...
            if let Some(op) = node.op() {
                match op {
                    ...
                    Op::Binary(lhs, rhs, BinaryOp::Div) => {
                        ...
                        grads.accumulate(rhs, &rhs_grad.neg()?)?;
                    }
                    ...
                    Op::Unary(arg, UnaryOp::Cos) => {
                        grads.accumulate(arg, &(&grad * arg.sin())?.neg()?)?
                    }
                    Op::Unary(arg, UnaryOp::Tanh) => {
                        let minus_dtanh = (node.sqr()? - 1.)?;
                        grads.accumulate(arg, &(&grad * &minus_dtanh)?.neg()?)?
                    }
...
impl GradStore {
    ...
    /// Add `grad` to the gradient of `tensor`. The first gradient is stored as it is, without
    /// materializing zeroes to add it to: a node reached once costs no fill and no add. A stored
    /// gradient is always contiguous, as the sum it replaces was.
