...
// dp4a decode -- the A/B knob (mirrors ROCm's IQ*_FALLBACK) and a production safety fallback.
// Read once. Set IQ_DEQUANT_FALLBACK=1 to disable the native i-quant mmvq decode path.
pub(crate) fn iq_dequant_fallback() -> bool {
    use std::sync::OnceLock;
    static EN: OnceLock<bool> = OnceLock::new();
    *EN.get_or_init(|| {
        matches!(
            std::env::var("IQ_DEQUANT_FALLBACK").as_deref(),
            Ok("1") | Ok("true") | Ok("TRUE")
        )
    })
}
...
fn indexed_moe_forward_fused_q8_1_input(
    ...
    // keeps the per-slot matvec (bandwidth-bound, capture-clean). MOE_QMMQ_FALLBACK forces the
    // per-slot path for the A/B; unsupported weight dtypes return None and fall through to per-slot.
    if batch > 1 && std::env::var("MOE_QMMQ_FALLBACK").is_err() {
