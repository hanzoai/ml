    ...
    fn from_float(xs: &[f32], ys: &mut [Self]) {
        ...
        for (i, y) in ys.iter_mut().enumerate() {
            ...
            if amax == 0f32 {
                ...
                // -127 rather than -128 keeps both extremes of the block representable, so
                // `+a` and `-a` quantize to -127/+127 instead of saturating one of them.
                // Matches ggml-quants.c `quantize_row_q8_K_ref`.
...
pub fn matmul<T: GgmlType>(
    ...
    let required_scratch_len = (m * act_blocks_per_row * elem_size).div_ceil(8);
    ...
    LHS_SCRATCH.with(|cell| -> Result<()> {
        ...
        for row_idx in 0..m {
            ...
            pool.execute_chunked(quads_total, |range| {
                ...
                for quad_idx in range {
                    ...
                    let (d0, d1, d2, d3) = T::vec_dot_4(
                        k,
                        &rhs_t[col * w..(col + 1) * w],
                        &rhs_t[(col + 1) * w..(col + 2) * w],
                        &rhs_t[(col + 2) * w..(col + 3) * w],
                        &rhs_t[(col + 3) * w..(col + 4) * w],
                        lhs_row,
                    );
                    unsafe {
            ...
            if n_tail & 1 == 1 {
                let col = n - 1;
                tail[n_tail - 1] = T::vec_dot(k, &rhs_t[col * w..(col + 1) * w], lhs_row);
            }
        }
        Ok(())
    })
