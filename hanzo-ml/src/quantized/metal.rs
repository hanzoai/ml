...
impl QMetalStorage {
    ...
    pub fn embedding(
        ...
        hanzo_metal_kernels::call_quantized_get_rows(
            device.device(),
            &encoder,
            device.kernels(),
            self.dtype.try_into()?,
    ...
    fn fwd_mv(
        ...
        let dst = device
            .new_buffer_builder()
            .with_size_for(dst_shape.elem_count(), out_dtype)
            .with_label("qmatmul")
            .build()?;
        let encoder = device.command_encoder()?;
    ...
    pub fn fwd(
        ...
        // Only the I/O dtype changes; the tile math is the same either way, so f32 models and
        // unsupported quants are unaffected.
        let src1_bf16 = storage.dtype() == DType::BF16
            && matches!(
                kdtype,
                hanzo_metal_kernels::GgmlDType::Q4K | hanzo_metal_kernels::GgmlDType::Q6K
