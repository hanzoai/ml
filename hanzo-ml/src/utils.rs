...
fn barrier_pool_spin_limit() -> u32 {
    std::env::var("BARRIER_POOL_SPIN_LIMIT")
        .ok()
        ...
        let threads: Vec<_> = (0..n_workers)
            .map(|tid| {
                ...
                std::thread::Builder::new()
                    ...
                    .expect("failed to spawn hanzo barrier pool worker")
            })
            .collect();
...
pub fn barrier_pool() -> &'static BarrierPool {
    BARRIER_POOL.get_or_init(|| BarrierPool::new(hanzo_num_threads().saturating_sub(1)))
}

pub fn with_threadpool<F: FnOnce() -> R + Send, R: Send>(f: F) -> R {
    hanzo_pool().install(f)
}
...
pub(crate) fn hanzo_pool() -> &'static rayon::ThreadPool {
    POOL.get_or_init(|| {
        rayon::ThreadPoolBuilder::new()
            .num_threads(get_num_threads())
            .start_handler(|_| set_thread_affinity())
            .build()
            .expect("failed to build hanzo rayon threadpool")
    })
}
