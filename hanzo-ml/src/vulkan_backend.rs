...
fn kernel_spv(name: &str) -> Result<&'static [u8]> {
    let b: &'static [u8] = match name {
        ...
        // Bindings: q(0), k(1), v(2), out(3), scale(4), meta(5)=[seq_q,seq_k,n_heads,n_kv,causal,
        // kv_batch_stride,kv_head_stride,key_stride,cube_base].
...
impl VulkanDevice {
    ...
    /// strides. `d`=128, `plane`(LocalSize)=32, BR=BC=16 are baked into the .spv. `q`/`out` are
...
mod dsl_dispatch_proof {
    ...
    // A/B: the fused coopmat flash (`flash_attn_dsl`) vs the shipped block-flash `sdpa_blk` -- the
    // current best Vulkan attention in this crate (the engine's split-decode `sdpa_decode_split`,
    // v1.7.68, is not in this ml branch) -- at the LIVE decode and prefill shapes, through the same ml
    // dispatch path. Both kernels are measured identically: q/k/v uploaded once, out/scale/meta
    // pre-allocated (no per-iter alloc), warmed, then a batch of N same-kernel dispatches timed with
    // one flush. Run under `VK_PROFILE_GPU=1` for the per-op on-GPU `[VK_GPU]` avg us corroboration.
    // This is a benchmark, not a gate; run explicitly (`--ignored`). It reports, never asserts speed.
    #[test]
    #[ignore]
