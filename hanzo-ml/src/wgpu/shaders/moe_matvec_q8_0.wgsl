// Fused MoE grouped quant matvec (Q8_0): y[s, r] = sum_k W[ids[s], r, k] * x[s, k], reading the
// per-expert slice from a GGML Q8_0 weight bank [E, n, k] in VRAM. Router gather + per-expert GEMM
// in one dispatch. Q8_0 block = 34 bytes = { f16 d ; i8 qs[32] }, weight = qs*d. One invocation =
// one y element. WGSL port of moe_matvec_q8_0.comp; params in a UNIFORM at binding 0.
struct Params { n: u32, k: u32, nrows: u32 };  // k mult of 32
@group(0) @binding(0) var<uniform> p: Params;
@group(0) @binding(1) var<storage, read>       w:   array<u32>;  // expert bank, raw Q8_0 bytes
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let expert = ids[s];
    let nblocks = p.k / 32u;
    let rowbase = (expert * p.n + r) * nblocks * 34u;
    let xbase = s * p.k;
    var acc: f32 = 0.0;
