...
pub struct MetalDevice {
    ...
    /// Heavily used by [`hanzo_metal_kernels`]
...
impl MetalDevice {
    /// Compile a raw MSL source string into a compute pipeline. cubecl-generated MSL (extracted via
    /// CUBECL_DEBUG_LOG, see tools/dsl/msl_extract.sh) drops in here, so a `#[kernel]` runs through the
    /// same Metal pipeline as every hand-written kernel -- the Metal twin of the Vulkan kernel_spv path.
    pub fn compile_msl(
        &self,
        func_name: &str,
        msl: &str,
    ...
    /// bookkeeping. Useful for buffers hanzo did not allocate, e.g.
