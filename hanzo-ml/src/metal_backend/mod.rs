...
impl BackendStorage for MetalStorage {
    ...
    fn where_cond(
        ...
        hanzo_metal_kernels::call_where_cond(
            &device.device,
            &encoder,
            &device.kernels,
            name,
    ...
    fn conv1d(
        ...
        let res = if kernel_l.is_contiguous() {
            ...
            // materialized kernel starting at offset 0, so that buffer, read at offset 0, is what
            // the matmul takes -- not the original strided one at its own start offset.
    ...
    fn upsample_bilinear2d(
        ...
        let src = buffer_o(&self.buffer, inp_l, self.dtype);
        hanzo_metal_kernels::call_upsample_bilinear_2d(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            name,
...
impl BackendDevice for MetalDevice {
    ...
    fn new(ordinal: usize) -> Result<Self> {
        ...
        #[cfg(feature = "metal-debug-labels")]
        command_queue.setLabel(Some(&NSString::from_str("hanzo-ml-metal")));
