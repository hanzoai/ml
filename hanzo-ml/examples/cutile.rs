...
use hanzo_ml::cutile;
use hanzo_ml::{CpuStorage, CudaStorage, CustomOp2, Layout, Result, Shape, Tensor};
...
impl CustomOp2 for Add {
    ...
    fn cpu_fwd(
        ...
        hanzo_ml::bail!("cutile-add requires a CUDA tensor")
    }
    ...
    fn cuda_fwd(
        ...
        if !lhs_layout.is_contiguous() || !rhs_layout.is_contiguous() {
            hanzo_ml::bail!("cutile-add requires contiguous tensors")
        }
        if lhs_layout.dims() != rhs_layout.dims() {
            hanzo_ml::bail!("cutile-add requires equal shapes")
        }
        let len = lhs_layout.shape().elem_count();
        if !len.is_multiple_of(BLOCK_SIZE) {
            hanzo_ml::bail!("cutile-add length must be divisible by {BLOCK_SIZE}")
        }
...
fn main() -> Result<()> {
    ...
    let device = hanzo_ml::Device::new_cuda(0)?;
    let lhs = Tensor::arange(0f32, BLOCK_SIZE as f32, &device)?;
    let rhs = Tensor::ones(BLOCK_SIZE, hanzo_ml::DType::F32, &device)?;
