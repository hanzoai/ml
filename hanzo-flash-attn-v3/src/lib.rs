...
//               2025 adjusted by Eric Buehler for hanzo repo.
...
impl FlashAttn {
    ...
    >(
        ...
        if head_size_og > 512 {
            hanzo::bail!("only supports head dimension at most 512 (got {head_size_og})")
        }
        if !(head_size_og == 512
            || head_size_og == 256
            || head_size_og == 128
            || head_size_og == 64)
        {
            hanzo::bail!("only supports head dimension 64, 128, 256 and 512 (got {head_size_og})")
        }
        if head_size_og % 8 != 0 {
...
impl hanzo::CustomOp3 for FlashAttn {
    ...
    fn cpu_fwd(
        ...
        hanzo::bail!("no cpu support for flash-attn-v3")
    }
    ...
    fn cuda_fwd(
        ...
        match q.dtype() {
            ...
            dt => hanzo::bail!("flash-attn-v3 is only supported for f16/bf16 ({dt:?})"),
        }
    }
}
