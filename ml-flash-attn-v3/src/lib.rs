...
use ml::cuda_backend::cudarc::driver::DevicePtr;
use ml::{CpuStorage, DType, Layout, Result, Shape, Tensor};
use half::{bf16, f16};
...
impl FlashAttn {
    fn cuda_fwd_t<
        T: ml::cuda_backend::CudaDType + ml::cuda_backend::cudarc::driver::DeviceRepr,
    >(
        &self,
        q: &ml::CudaStorage,
        q_l: &Layout,
        ...
        if q_stride[q_rank - 1] != 1 {
            ml::bail!("the last dim of q must be contiguous {q_stride:?}")
        }
        if k_stride[k_rank - 1] != 1 {
            ml::bail!("the last dim of k must be contiguous {k_stride:?}")
        }
        if v_stride[v_rank - 1] != 1 {
            ml::bail!("the last dim of v must be contiguous {v_stride:?}")
        }
        ...
        if expected_kv != k_l.shape().dims4()? {
            ml::bail!("shape mismatch q {:?} and k {:?}", q_l.shape(), k_l.shape())
        }
        if expected_kv != v_l.shape().dims4()? {
            ml::bail!("shape mismatch q {:?} and v {:?}", q_l.shape(), v_l.shape())
        }
        if head_size_og > 256 {
            ml::bail!("only supports head dimension at most 256 (got {head_size_og})")
        }
        if !(head_size_og == 256 || head_size_og == 128 || head_size_og == 64) {
            ml::bail!("only supports head dimension 64, 128 and 256 (got {head_size_og})")
        }
        if head_size_og % 8 != 0 {
            // TODO: Handle head sizes that are not a multiple of 8 via some padding.
            ml::bail!("only supports head sizes that are a multiple of 8 (got {head_size_og})")
        }
        if num_heads % num_heads_k != 0 {
            ml::bail!("number of k/v heads {num_heads_k} must divide number of heads in query {num_heads}")
        }
        ...
        let alibi_slopes_ptr = if let Some(alibi_slopes) = &self.alibi_slopes {
            ...
            let alibi_slopes = match &*alibi_slopes {
                ml::Storage::Cuda(c) => c.as_cuda_slice::<f32>()?,
                _ => ml::bail!("alibi_slopes must be a cuda tensor"),
            };
        ...
        let dst = ml::CudaStorage::wrap_cuda_slice(dst, dev.clone());
        Ok((dst, out_shape))
    }
}

impl ml::CustomOp3 for FlashAttn {
    ...
    fn cpu_fwd(
        ...
        ml::bail!("no cpu support for flash-attn-v3")
    }
    ...
    fn cuda_fwd(
        ...
        match q.dtype() {
            ml::DType::F16 => self.cuda_fwd_t::<f16>(q, q_l, k, k_l, v, v_l, false),
            ml::DType::BF16 => self.cuda_fwd_t::<bf16>(q, q_l, k, k_l, v, v_l, true),
            dt => ml::bail!("flash-attn-v3 is only supported for f16/bf16 ({dt:?})"),
        }
    }
}
...
impl FlashAttnVarLen {
    ...
    >(
        ...
        let seqlens_q = match &*seqlens_q {
            ml::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?, // Should be i32!
            _ => ml::bail!("seqlens_q must be a cuda tensor"),
        };
        let seqlens_q = match seqlens_q_layout.contiguous_offsets() {
            Some((o1, o2)) => seqlens_q.slice(o1..o2),
            None => ml::bail!("seqlens_q has to be contiguous"),
        };

        let (seqlens_k, seqlens_k_layout) = self.seqlens_k.storage_and_layout();
        let seqlens_k = match &*seqlens_k {
            ml::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?, // Should be i32!
            _ => ml::bail!("seqlens_k must be a cuda tensor"),
        };
        let seqlens_k = match seqlens_k_layout.contiguous_offsets() {
            Some((o1, o2)) => seqlens_k.slice(o1..o2),
            None => ml::bail!("seqlens_k has to be contiguous"),
        };
        ...
        if nseqlens_q < 2 {
            ml::bail!("seqlens_q should have a len >= 2 {nseqlens_q}")
        }
        let nseqlens_k = seqlens_k_layout.shape().dims1()?;
        if nseqlens_k != nseqlens_q {
            ml::bail!("seqlens_q and seqlens_k should have the same number of elements {nseqlens_q} <> {nseqlens_k}")
        }
...
impl ml::CustomOp3 for FlashAttnVarLen {
    fn name(&self) -> &'static str {
