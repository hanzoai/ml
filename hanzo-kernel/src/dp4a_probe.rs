...
fn dp4a_kernel(
    a: &Array<Vector<i32, Const<4>>>,
    b: &Array<Vector<i32, Const<4>>>,
    out: &mut Array<i32>,
) {
    let i = ABSOLUTE_POS;
    if i < out.len() {
...
fn main() {
    ...
    // reference
    let refv: Vec<i32> = (0..n)
        .map(|i| (0..4).map(|j| a[i * 4 + j] * b[i * 4 + j]).sum())
        .collect();
    ...
    use hanzo_kernel::quant::{
        matvec_q8_bench, matvec_q8_dp4a_ref, matvec_q8_dp4a_run, matvec_q8_ref,
    };
    ...
    let wd: Vec<f32> = (0..rows * nb)
        .map(|_| (rnd() % 1000) as f32 / 8000.0 + 0.01)
        .collect();
    ...
    for (a, b) in reference.iter().zip(got.iter()) {
        mr = mr.max((a - b).abs() / a.abs().max(1.0));
    }
