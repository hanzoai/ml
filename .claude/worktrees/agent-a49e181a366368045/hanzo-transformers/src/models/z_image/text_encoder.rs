...
#[derive(Debug, Clone)]
struct Mlp {
    gate_proj: hanzo_nn::Linear,
    up_proj: hanzo_nn::Linear,
    down_proj: hanzo_nn::Linear,
    act_fn: Activation,
}
...
impl Mlp {
    fn new(cfg: &TextEncoderConfig, vb: VarBuilder) -> Result<Self> {
        Ok(Self {
            gate_proj: hanzo_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.intermediate_size,
                vb.pp("gate_proj"),
            )?,
            up_proj: hanzo_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.intermediate_size,
                vb.pp("up_proj"),
            )?,
            down_proj: hanzo_nn::linear_no_bias(
                cfg.intermediate_size,
                cfg.hidden_size,
                vb.pp("down_proj"),
            )?,
...
impl ZImageTextEncoder {
    pub fn new(cfg: &TextEncoderConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let embed_tokens =
            hanzo_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb_model.pp("embed_tokens"))?;
    ...
    pub fn forward(&self, input_ids: &Tensor) -> Result<Tensor> {
        ...
        hanzo_ml::bail!("Layer index out of bounds")
    }
