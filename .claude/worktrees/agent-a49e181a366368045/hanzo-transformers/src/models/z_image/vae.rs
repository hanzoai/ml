...
use hanzo_nn::{conv2d, group_norm, Conv2d, Conv2dConfig, GroupNorm, VarBuilder};
...
impl Attention {
    fn new(channels: usize, num_groups: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let to_q = hanzo_nn::linear(channels, channels, vb.pp("to_q"))?;
        let to_k = hanzo_nn::linear(channels, channels, vb.pp("to_k"))?;
        let to_v = hanzo_nn::linear(channels, channels, vb.pp("to_v"))?;
        let to_out = hanzo_nn::linear(channels, channels, vb.pp("to_out").pp("0"))?;
        Ok(Self {
            group_norm,
            to_q,
            to_k,
