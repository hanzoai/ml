...
_UNSIGNED_DTYPES = set([str(hanzo.u8), str(hanzo.u32)])
...
def assert_almost_equal(
    ...
    if str(actual.dtype) in _UNSIGNED_DTYPES or str(expected.dtype) in _UNSIGNED_DTYPES:
        actual = actual.to(hanzo.i64)
        expected = expected.to(hanzo.i64)

    diff = (actual - expected).abs()

    threshold = (expected.abs().to_dtype(hanzo.f32) * rtol + atol).to(expected)
