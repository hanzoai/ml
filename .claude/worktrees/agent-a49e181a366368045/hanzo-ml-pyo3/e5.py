from hanzo.utils import load_safetensors, save_gguf, load_gguf
from hanzo.models.bert import BertModel, Config
import json
...
if __name__ == "__main__":
    ...
    hanzo_pooled = average_pool(torch.tensor(encoder_out.values()), hf_tokenized["attention_mask"])

    loss = torch.nn.L1Loss()
    error = loss(hf_pooled, hanzo_pooled).mean().item()
    print(f"Mean error between torch-reference and hanzo: {error}")
    ...
    hanzo_pooled_2 = average_pool(torch.tensor(encoder_out_2.values()), hf_tokenized["attention_mask"])
    error = loss(hf_pooled, hanzo_pooled_2).mean().item()
    print(f"Mean error between torch-reference and quantized-hanzo: {error}")
