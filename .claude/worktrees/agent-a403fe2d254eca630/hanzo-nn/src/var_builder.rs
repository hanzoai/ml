...
use crate::VarMap;
use hanzo_ml::{safetensors::Load, DType, Device, Error, Result, Shape, Tensor};
...
impl SimpleBackend for VarMap {
    ...
    fn get_unchecked(&self, _name: &str, _dtype: DType, _dev: &Device) -> Result<Tensor> {
        hanzo_ml::bail!("`get_unchecked` does not make sense for `VarMap`, use `get`.");
    }
...
impl SimpleBackend for hanzo_ml::npy::NpzTensors {
    fn get(
        &self,
        s: Shape,
        path: &str,
...
impl SimpleBackend for hanzo_ml::pickle::PthTensors {
    fn get(
        &self,
        s: Shape,
        path: &str,
...
impl SimpleBackend for hanzo_ml::safetensors::MmapedSafetensors {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl SimpleBackend for hanzo_ml::safetensors::BufferedSafetensors {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl SimpleBackend for hanzo_ml::safetensors::SliceSafetensors<'_> {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl<'a> VarBuilder<'a> {
    ...
    pub unsafe fn from_mmaped_safetensors<P: AsRef<std::path::Path>>(
        ...
        let tensors = hanzo_ml::safetensors::MmapedSafetensors::multi(paths)?;
    ...
    pub fn from_buffered_safetensors(data: Vec<u8>, dtype: DType, dev: &Device) -> Result<Self> {
        let tensors = hanzo_ml::safetensors::BufferedSafetensors::new(data)?;
        Ok(Self::from_backend(Box::new(tensors), dtype, dev.clone()))
    }
    ...
    pub fn from_slice_safetensors(data: &'a [u8], dtype: DType, dev: &Device) -> Result<Self> {
        let tensors = hanzo_ml::safetensors::SliceSafetensors::new(data)?;
        Ok(Self::from_backend(Box::new(tensors), dtype, dev.clone()))
    }
    ...
    pub fn from_npz<P: AsRef<std::path::Path>>(p: P, dtype: DType, dev: &Device) -> Result<Self> {
        let npz = hanzo_ml::npy::NpzTensors::new(p)?;
    ...
    pub fn from_pth<P: AsRef<std::path::Path>>(p: P, dtype: DType, dev: &Device) -> Result<Self> {
        let pth = hanzo_ml::pickle::PthTensors::new(p, None)?;
    ...
    pub fn from_pth_with_state<P: AsRef<std::path::Path>>(
        ...
        let pth = hanzo_ml::pickle::PthTensors::new(p, Some(state_key))?;
        Ok(Self::from_backend(Box::new(pth), dtype, dev.clone()))
    }
...
pub struct ShardedSafeTensors(hanzo_ml::safetensors::MmapedSafetensors);
...
impl Backend for ShardedSafeTensors {
    ...
    fn get(
        ...
        let iterator = if dim == 0 {
            ...
            hanzo_ml::bail!("Get sharded on dimensions != 0 or 1")
        };
    ...
    fn get_unchecked(&self, _name: &str, _dtype: DType, _dev: &Device) -> Result<Tensor> {
        hanzo_ml::bail!("`get_unchecked` does not make sense for `ShardedSafeTensors`, use `get`.");
    }
