...
cargo run --features cuda --release --example paligemma -- \
    --prompt "caption fr" --image hanzo-ml-examples/examples/yolo-v8/assets/bike.jpg
```
...
```bash
cargo run --features cuda --release --example paligemma -- \
    --prompt "caption fr" --image hanzo-ml-examples/examples/flux/assets/flux-robot.jpg
```
