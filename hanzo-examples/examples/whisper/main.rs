...
use hanzo_transformers::models::whisper::{self as m, audio, Config};
...
fn main() -> Result<()> {
    ...
    let (config_filename, tokenizer_filename, weights_filename, input) = {
        let api = Api::new()?;
        let dataset = api.dataset("Narsil/hanzo-examples".to_string());
    ...
    let (pcm_data, sample_rate) = hanzo_examples::audio::pcm_decode(input)?;
