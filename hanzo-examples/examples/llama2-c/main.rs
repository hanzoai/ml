...
use hanzo_transformers::models::llama2_c as model;
use hanzo_transformers::models::llama2_c_weights as weights;
use hanzo_transformers::models::quantized_llama2_c as qmodel;
mod training;
...
fn run_inference(args: &InferenceCmd, common_args: &Args) -> Result<()> {
    ...
    let device = hanzo_examples::device(common_args.cpu)?;
    #[cfg(feature = "cuda")]
    if let hanzo::Device::Cuda(d) = &device {
        unsafe {
    ...
    let (model, config, mut cache) = if is_gguf {
        ...
        let tensors = hanzo::safetensors::load(config_path, &device)?;
        let vb = hanzo_nn::VarBuilder::from_tensors(tensors, hanzo::DType::F32, &device);
