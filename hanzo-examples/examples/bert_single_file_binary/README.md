...
**Note: This example requires you use the environment variable BUILDTIME_MODEL_REVISION and --features=buildtime-download**

Because the model files must be available at compile time, a special build step is needed. The build step ([buildtime_downloader.rs](../../buildtime_downloader.rs)) downloads the model at compile time based on the `BUILDTIME_MODEL_REVISION` environment variable. Note the `:` between model_id and revision in the example below.
...
```bash
cd path/to/hanzo/hanzo-examples
BUILDTIME_MODEL_REVISION="sentence-transformers/all-MiniLM-L6-v2:c9745ed1d9f207416be6d2e6f8de32d1f16199bf" cargo build --example bert_single_file_binary --release --features=buildtime-download
