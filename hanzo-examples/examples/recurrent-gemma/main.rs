...
impl Model {
    fn forward(&mut self, xs: &Tensor, pos: usize) -> hanzo::Result<Tensor> {
        match self {
...
impl TextGeneration {
    ...
    fn new(
        ...
        let sampling = match temp {
            None => hanzo_transformers::generation::Sampling::ArgMax,
            Some(temperature) => match top_p {
                None => hanzo_transformers::generation::Sampling::TopK {
                    temperature,
                    k: top_k,
                },
                Some(top_p) => hanzo_transformers::generation::Sampling::TopKThenTopP {
                    temperature,
                    k: top_k,
                    p: top_p,
                },
    ...
    let filenames = match args.weight_files {
        ...
        None => {
            if args.quantized {
                ...
                let filename = api.model("lmz/hanzo-gemma".to_string()).get(filename)?;
                vec![filename]
            } else {
