...
pub enum MetalError {
    ...
    Message(String),
    #[error(transparent)]
    KernelError(#[from] ml_metal_kernels::MetalKernelError),
    #[error("{0:?}")]
    LockError(LockError),
...
impl BackendStorage for MetalStorage {
    ...
    fn affine(&self, layout: &Layout, mul: f64, add: f64) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            ml_metal_kernels::call_affine_strided(
                &device.device,
                &encoder,
                &device.kernels,
                name,
    ...
    fn powf(&self, layout: &Layout, pow: f64) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            ml_metal_kernels::call_powf_strided(
                &device.device,
                &encoder,
                &device.kernels,
                name,
    ...
    fn elu(&self, layout: &Layout, alpha: f64) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            ml_metal_kernels::call_elu_strided(
                &device.device,
                &encoder,
                &device.kernels,
                name,
    ...
    fn reduce_op(&self, op: ReduceOp, layout: &Layout, sum_dims: &[usize]) -> Result<Self> {
        ...
        if layout.is_contiguous() && reduction_shape.is_contiguous(&stride) {
            ...
            let src = buffer_o(&self.buffer, layout, self.dtype);
            ml_metal_kernels::call_reduce_contiguous(
                &device.device,
                &encoder,
                &device.kernels,
                name,
        ...
        encoder.set_label("reduce");
        let src = buffer_o(&self.buffer, layout, self.dtype);
        ml_metal_kernels::call_reduce_strided(
            &device.device,
            &encoder,
            &device.kernels,
            name,
    ...
    fn const_set(&mut self, s: crate::scalar::Scalar, l: &Layout) -> Result<()> {
        ...
        fn set<S: crate::WithDType + ml_metal_kernels::utils::EncoderParam>(
            ...
            if l.is_contiguous() {
                ...
                ml_metal_kernels::call_const_set_contiguous(
                    &device.device,
                    &encoder,
                    &device.kernels,
                    kernel_name,
                ...
                ml_metal_kernels::call_const_set_strided(
                    &device.device,
                    &encoder,
                    &device.kernels,
                    kernel_name,
    ...
    fn to_dtype(&self, layout: &Layout, dtype: DType) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            ml_metal_kernels::call_cast_contiguous(
                &device.device,
                &encoder,
                &device.kernels,
                kernel_name,
            ...
            ml_metal_kernels::call_cast_strided(
                &device.device,
                &encoder,
                &device.kernels,
                kernel_name,
    ...
    fn where_cond(
        ...
        ml_metal_kernels::call_where_cond(
            &device.device,
            &encoder,
            &device.kernels,
            name,
    ...
    fn conv1d(
        ...
        let src = buffer_o(&self.buffer, layout, self.dtype);
        ml_metal_kernels::call_im2col1d_strided(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    fn conv_transpose1d(
        ...
        let buffer = if USE_COL2IM_CONV1D_TR && can_use_col2im {
            ...
            ml_metal_kernels::call_col2im1d(
                &self.device.device,
                &encoder,
                &self.device.kernels,
                name,
            ...
            ml_metal_kernels::call_conv_transpose1d(
                &self.device.device,
                &encoder,
                &self.device.kernels,
                name,
    ...
    fn conv2d(
        ...
        let src = buffer_o(&self.buffer, layout, self.dtype);
        ml_metal_kernels::call_im2col_strided(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    fn conv_transpose2d(
        ...
        ml_metal_kernels::call_conv_transpose2d(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    fn upsample_nearest2d(&self, inp_l: &Layout, out_w: usize, out_h: usize) -> Result<Self> {
        ...
        ml_metal_kernels::call_upsample_nearest_2d(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    fn upsample_bilinear2d(
        ...
        let src = buffer_o(&self.buffer, inp_l, self.dtype);
        ml_metal_kernels::call_upsample_bilinear_2d(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    fn index_select(&self, ids: &Self, src_l: &Layout, ids_l: &Layout, dim: usize) -> Result<Self> {
        ...
        let src = buffer_o(&self.buffer, src_l, dtype);
        let ids = buffer_o(&ids.buffer, ids_l, ids.dtype);
        ml_metal_kernels::call_index_select(
            &device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    fn index_add(
        ...
        let src = buffer_o(&src.buffer, src_l, src.dtype);
        let ids = buffer_o(&ids.buffer, ids_l, ids.dtype);
        ml_metal_kernels::call_index_add(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    fn matmul(
        ...
        let dtype = match self.dtype {
            DType::F32 => ml_metal_kernels::GemmDType::F32,
            DType::F16 => ml_metal_kernels::GemmDType::F16,
            DType::BF16 => ml_metal_kernels::GemmDType::BF16,
            dtype => {
                return Err(
        ...
        ml_metal_kernels::call_mlx_gemm(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            dtype,
    ...
    fn copy2d(
        ...
        if src_s == d2 && dst_s == d2 {
            ...
            let kernel_name = match self.dtype {
                DType::F32 => ml_metal_kernels::copy2d::FLOAT,
                DType::F16 => ml_metal_kernels::copy2d::HALF,
                DType::BF16 => ml_metal_kernels::copy2d::BFLOAT,
                DType::I64 => ml_metal_kernels::copy2d::I64,
                DType::U32 => ml_metal_kernels::copy2d::U32,
                DType::U8 => ml_metal_kernels::copy2d::U8,
                dtype => crate::bail!("Metal copy2d {dtype:?} not implemented"),
            };
            let encoder = self.device.command_encoder()?;
            encoder.set_label("copy2d");
            ml_metal_kernels::call_copy2d(
                &self.device.device,
                &encoder,
                &self.device.kernels,
                kernel_name,
    ...
    fn copy_strided_src(&self, dst: &mut Self, dst_offset: usize, src_l: &Layout) -> Result<()> {
        if src_l.is_contiguous() && self.dtype == dst.dtype() {
            ...
            let kernel_name = match self.dtype {
                DType::F32 => ml_metal_kernels::unary::strided::copy::FLOAT,
                DType::F16 => ml_metal_kernels::unary::strided::copy::HALF,
                DType::BF16 => ml_metal_kernels::unary::strided::copy::BFLOAT,
                DType::I64 => ml_metal_kernels::unary::strided::copy::I64,
                DType::U32 => ml_metal_kernels::unary::strided::copy::U32,
                DType::U8 => ml_metal_kernels::unary::strided::copy::U8,
...
impl MetalStorage {
    ...
    pub fn binary(
        ...
        let buffer = if lhs_contiguous && rhs_contiguous {
            ...
            ml_metal_kernels::call_binary_contiguous(
                &device.device,
                &encoder,
                &device.kernels,
                kernel,
            ...
            let buffer = device.new_buffer(el_count, dtype, op)?;
            ml_metal_kernels::call_binary_strided(
                &device.device,
                &encoder,
                &device.kernels,
                kernel,
...
impl BackendDevice for MetalDevice {
    ...
    fn rand_uniform(
        ...
        ml_metal_kernels::call_random_uniform(
            &self.device,
            &encoder,
            &self.kernels,
            name,
    ...
    fn rand_normal(
        ...
        ml_metal_kernels::call_random_normal(
            &self.device,
            &encoder,
            &self.kernels,
            name,
