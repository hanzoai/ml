...
// Default Vulkan custom-op path: download arg(s) to CPU, run the CPU forward, re-upload.
// Correct for every op; ops with a native SPIR-V kernel override `vulkan_fwd` to skip this.
fn vulkan_cpu_fallback1(
    _name: &str,
    s: &crate::VulkanStorage,
    f: impl FnOnce(&CpuStorage) -> Result<(CpuStorage, Shape)>,
) -> Result<(crate::VulkanStorage, Shape)> {
    use crate::backend::{BackendDevice, BackendStorage};
    let cpu = s.to_cpu_storage()?;
    let (out, shape) = f(&cpu)?;
    let v = s.device().clone().storage_from_cpu_storage(&out)?;
    Ok((v, shape))
}

fn vulkan_cpu_fallback2(
    _name: &str,
    s1: &crate::VulkanStorage,
    s2: &crate::VulkanStorage,
    f: impl FnOnce(&CpuStorage, &CpuStorage) -> Result<(CpuStorage, Shape)>,
) -> Result<(crate::VulkanStorage, Shape)> {
    use crate::backend::{BackendDevice, BackendStorage};
    let c1 = s1.to_cpu_storage()?;
    let c2 = s2.to_cpu_storage()?;
    let (out, shape) = f(&c1, &c2)?;
    let v = s1.device().clone().storage_from_cpu_storage(&out)?;
    Ok((v, shape))
}

fn vulkan_cpu_fallback3(
    _name: &str,
    s1: &crate::VulkanStorage,
    s2: &crate::VulkanStorage,
    s3: &crate::VulkanStorage,
    f: impl FnOnce(&CpuStorage, &CpuStorage, &CpuStorage) -> Result<(CpuStorage, Shape)>,
) -> Result<(crate::VulkanStorage, Shape)> {
    use crate::backend::{BackendDevice, BackendStorage};
    let c1 = s1.to_cpu_storage()?;
    let c2 = s2.to_cpu_storage()?;
    let c3 = s3.to_cpu_storage()?;
    let (out, shape) = f(&c1, &c2, &c3)?;
    let v = s1.device().clone().storage_from_cpu_storage(&out)?;
    Ok((v, shape))
}
...
pub trait CustomOp1 {
    ...
    /// The forward pass on a Vulkan device. The default downloads to CPU, runs `cpu_fwd`, and
    /// re-uploads (correct but slow); ops with a native SPIR-V kernel override this.
    fn vulkan_fwd(
        &self,
        storage: &crate::VulkanStorage,
        layout: &Layout,
    ) -> Result<(crate::VulkanStorage, Shape)> {
        vulkan_cpu_fallback1(self.name(), storage, |s| self.cpu_fwd(s, layout))
    }
...
pub trait CustomOp2 {
    ...
    /// Vulkan forward; default CPU round-trip (see `CustomOp1::vulkan_fwd`).
    fn vulkan_fwd(
        &self,
        s1: &crate::VulkanStorage,
        l1: &Layout,
        ...
        vulkan_cpu_fallback2(self.name(), s1, s2, |c1, c2| self.cpu_fwd(c1, l1, c2, l2))
    }
...
pub trait CustomOp3 {
    ...
    fn vulkan_fwd(
        ...
        vulkan_cpu_fallback3(self.name(), s1, s2, s3, |c1, c2, c3| {
            self.cpu_fwd(c1, l1, c2, l2, c3, l3)
        })
    }
...
pub struct UgIOp1 {
    ...
    #[cfg(feature = "metal")]
    func: ml_metal_kernels::metal::ComputePipeline,
}
...
impl InplaceOp1 for UgIOp1 {
    ...
    fn metal_fwd(&self, sto: &mut MetalStorage, layout: &Layout) -> Result<()> {
        ...
        let group_dims = ml_metal_kernels::utils::get_block_dims(b, 1, 1);
        ml_metal_kernels::utils::set_param(&encoder, 0, (sto.buffer(), 0usize));
