use ml::{test_device, Device, IndexOp, Result, Tensor};
use ml_core as ml;
...
fn strided_blocks() -> Result<()> {
    ...
    match tensor.strided_blocks() {
        ml::StridedBlocks::SingleBlock { start_offset, len } => {
            assert_eq!(start_offset, 0);
            assert_eq!(len, 24);
        }
        ml::StridedBlocks::MultipleBlocks { .. } => {
