...
use ml_core::{test_device, test_utils, DType, Device, Shape, Tensor, Var};
...
fn test_flip_backprop() -> Result<()> {
    ...
    ml_core::test_utils::assert_tensor_eq(&y, &expected_y)?;

    let z = y.flip(&[1])?;
    let expected_z = Tensor::from_vec(vec![6.0, 4.0, 6.0, 4.0], (2, 2), device)?;
    ml_core::test_utils::assert_tensor_eq(&z, &expected_z)?;
    ...
    ml_core::test_utils::assert_tensor_eq(grad_x, &expected_grad)?;
