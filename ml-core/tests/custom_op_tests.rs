use ml_core::backend::BackendStorage;
use ml_core::cpu_backend;
use ml_core::test_utils::to_vec1_round;
use ml_core::{CpuStorage, CustomOp1, DType, Device, Error, Layout, Result, Shape, Tensor};
...
fn custom_op1_with_backward() -> Result<()> {
    let cpu = &Device::Cpu;
    let t = ml_core::Var::new(&[-2f32, 0f32, 2f32], cpu)?;
...
impl ml_core::InplaceOp1 for Elu {
    fn name(&self) -> &'static str {
        "elu"
    }
    ...
    fn cpu_fwd(&self, s: &mut CpuStorage, _l: &Layout) -> Result<()> {
        ...
        match s {
            ...
            _ => ml_core::bail!("unsupported dtype for inplace elu"),
        }
        Ok(())
    }
}
...
fn ug_op() -> Result<()> {
    let kernel = {
        ...
        let layout = ml_ug::Layout::from_shape(&[12]);
        let ptr = op::Arg::ptr(ml_ug::DType::F32);
        let src = op::load(ptr.id(), layout.clone(), ml_ug::DType::F32)?;
        ...
        let opts: ml_ug::lower_op::Opts = Default::default();
        kernel.lower(&opts)?
    };
    let device = if ml_core::utils::cuda_is_available() {
        Device::new_cuda(0)?
    } else if ml_core::utils::metal_is_available() {
        Device::new_metal(0)?
    } else {
        ml_core::bail!("metal/cuda is mandatory for this test")
    };
    let op = ml_core::UgIOp1::new("test", kernel, &device)?;
