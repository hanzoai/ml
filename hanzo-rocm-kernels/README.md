...
```
~/.cache/hanzo-rocm/{arch}-{rocm_version}/
```

For example:
- `~/.cache/hanzo-rocm/gfx908-6.1/binary_a1b2c3d4.cso`
- `~/.cache/hanzo-rocm/gfx942-6.2/binary_a1b2c3d4.cso`
...
- **GPU Detection**: Automatically detects GPU architecture using `rocminfo` or falls back to environment variable `ROCM_ARCH`
- **Version Detection**: Detects ROCm version using `hipcc --version` or environment variable `ROCM_VERSION`
...
- `ROCM_ARCH` - Override GPU architecture detection (e.g., "gfx908")
- `ROCM_VERSION` - Override ROCm version detection (e.g., "6.1")
...
The system will try to auto-detect, but you can override with `ROCM_ARCH`.
